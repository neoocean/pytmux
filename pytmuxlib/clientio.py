...
class _RenderMixin:
    ...
    def _composite(self):
        ...
        # 중앙선) 자리를 셀 높이 전체로 덮어 아웃라인을 침범한다(제보).
    ...
    def push_screen(self, *args, **kwargs):
        ...
        if getattr(self, "view", None) is not None:
            ...
            # 상태표시줄·탭바는 _composite 그리드 밖의 별도 위젯이라 backdrop 딤 중 자기
            # 이모지(⚠·탭명)를 placeholder 로 바꾼다(#25). 모달이 떠도 캐시 strip 을
            # 재사용하지 않게 명시 refresh — 안 하면 이모지가 딤 안 된 채 남는다.
            for _w in (getattr(self, "status", None), getattr(self, "tabbar", None)):
                if _w is not None:
                    _w.refresh()
        return r
    ...
    def pop_screen(self, *args, **kwargs):
        ...
        if getattr(self, "view", None) is not None:
            self._composite()
            self.call_after_refresh(self._composite)
            # 팝업이 닫히면 상태표시줄·탭바도 원본 이모지로 되돌리도록 재렌더(#25, push 대칭).
            for _w in (getattr(self, "status", None), getattr(self, "tabbar", None)):
                if _w is not None:
                    _w.refresh()
        return r
