...
class ServerTreeMixin:
    ...
    def _drive_roots() -> list[str]:
        ...
        그 외 OS: [](단일 루트 '/'). ncd 가 **드라이브 전환**을 위해 드라이브들을
        트리 최상위로 묶는 데 쓴다."""
        if os.name != "nt":
            return []
        try:
    ...
    def _build_chain(self, chain_paths: list[str], drives: list[str]) -> list[list]:
        """루트→cwd 사슬을 [dir, [직계 하위]] 리스트로 만든다. `drives` 가 있으면
        (Windows) 맨 앞에 **합성 최상위('')**를 두고 그 자식으로 드라이브 목록을 실어
        드라이브를 트리 최상위 노드로 만든다(현재 드라이브가 빠지면 보강). 각 단계엔
        다음 사슬 원소를 보장 포함(숨김/누락 대비)해 펼친 경로가 끊기지 않게 한다."""
        chain: list[list] = []
        if drives:
    ...
    def nc_list_msg(self, sess: Session, path: str | None = None) -> dict:
        ...
        - `path` 가 비면(초기 진입) **루트부터 현재 패널 cwd 까지의 사슬**을 만들어
          ...
          **Windows** 에선 드라이브 문자들을 묶는 합성 최상위('')를 맨 위에 둬
          드라이브 전환이 되게 한다(`root`=""). cwd 추정 불가 시 루트만 1단계.
        ...
        drives = self._drive_roots()
        chain = self._build_chain(chain_paths, drives)
