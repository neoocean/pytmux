...
def _relay_allowed(sock_path: str, host: str) -> bool:
    ...
    **중계자가 자기 egress 정책의 주인**이라는 것이 이 함수의 존재 이유다. 안 그러면
    ...
    B 의 서버와 무관하다) 파일만 있으면 정책이 선다.

    ★ **목록이 비면 거부한다**(fail-closed, 2026-08-01 · §10-16ⓔ). 종전에는 비면
    허용이었다 — "정책 미설정"을 "전부 허용"으로 읽은 것이고, 그 기본값에서 B 는
    자기가 도약대로 쓰이는 줄도 모른 채 아무 데나 나간다. 이 함수가 지키는 것이
    **B 의 egress** 라, 모르면 안 나가는 쪽이 맞는 기본값이다.

    바꿔도 되는 이유(호환): 이 경로는 **다중홉 릴레이 전용**이다(`relay-proxy`).
    평범한 원격 attach(A→B)는 여기를 지나지 않으므로 기존 사용자의 동선은 그대로다.
    다중홉은 아직 웹·가이드에 싣지 않은 미공개 기능이라(§10-15 라이브 3대 미검증)
    지금이 기본값을 바로잡는 가장 싼 시점이다."""
    try:
...
# ⚠ 2026-08-01: `--native`(러스트 TUI 를 띄우던 플래그)와 그 탐색·실행 함수를 지웠다.
# 클라 제품은 **정본 Textual 클라와 warp 기반 GUI 둘**이다(사용자 결정 —
# docs/internal/CLIENT_PRODUCT_SET_2026-08-01.md). GUI 는 `pytmux-gui` 를 직접 실행한다.
