"""크로스플랫폼 PTY 백엔드 추상층 (docs/internal/WINDOWS_PORT.md §6-1 ①②).
...
# owned ConPTY → pywinpty 폴백을 처음 한 번만 stderr 로 알리는 가드(H3, 매 탭 spam 방지).
_OWNED_FALLBACK_LOGGED: list = []
...
def adopt(fd: int, pid: int, *, cols: int, rows: int) -> "PtyProcess":
    ...
    (docs/internal/RESTART_SCENARIO.md ⓓ). PID 가 그대로라 reap/killpg 의미가 유효하다.
    ...
    if IS_WINDOWS:
        ...
        if choice not in ("pywinpty", "winpty"):
            try:
                ...
                # H3: 종전엔 무로깅 pass 라 owned 백엔드의 영구 실패(매 spawn 폴백 +
                # 누수)가 은폐됐다. 첫 폴백 1회만 stderr 에 원인을 남긴다(매 탭 spam
                # 방지) — 폴백 자체는 정상 동작이므로 동작은 그대로 pywinpty 로 이어간다.
                if not _OWNED_FALLBACK_LOGGED:
                    _OWNED_FALLBACK_LOGGED.append(True)
                    import sys as _sys
                    import traceback as _tb
                    print("pytmux: owned ConPTY 백엔드 실패 → pywinpty 폴백",
                          file=_sys.stderr)
                    _tb.print_exc()
...
class _OwnedConPty(PtyProcess):
    ...
    def __init__(self, argv, *, cols: int, rows: int, cwd, env):
        ...
        # 워밍 풀(§6A): 미리 만든 의사콘솔이 있으면 꺼내 요청 크기로 resize 후 셸을
        # attach 한다 — CreatePseudoConsole(OpenConsole 호스트 spawn ~90ms)을 건너뛰어
        # 새 탭 비용이 ~절반. cwd/argv 는 여기서 주므로 풀 객체는 내용 무관해 안전하다.
        # 풀이 비었거나 꺼져 있으면 종전대로 새로 만든다. 어느 쪽이든 비동기 리필을
        # 트리거해 다음 spawn 이 적중하게 한다.
        cp = conpty._pool_take()
        if cp is not None:
            cp.resize(max(1, cols), max(1, rows))
        else:
            cp = conpty._ConPty(max(1, cols), max(1, rows))
        conpty._pool_refill_async()
        try:
            cp.spawn(list(argv), cwd=cwd, env=env)
        except BaseException:
            # H3: spawn(CreateProcessW 등) 실패 시 cp(HPCON + conin/conout 파이프 +
            # OpenConsole 호스트)를 닫아 핸들/프로세스 누수를 막는다. self._cp 가 아직
            # 미설정이라 close() 정리 경로가 안 닿으므로 여기서 직접 회수한다.
            try:
                cp.close()
            except Exception:
                pass
