...
# 정본(파이썬 Textual) 클라가 hello 에 실어 광고하는 능력. 서버는 여기 있는 것만
# 보낸다 — 광고 안 한 프레임은 한 바이트도 안 온다(§10-13 계약).
#
# ⛔ **그리지도 파싱하지도 않을 것을 넣지 말 것.** 이 목록은 "받고 싶다"가 아니라
# "이미 쓰는 곳이 있다"는 선언이다.
#
# ★ **여기 있는 것이 전부가 아니다** — 플러그인이 `client_caps` 로 기여한 것이 여기에
# 합쳐져 hello 에 실린다(`Registry.client_caps`). `blocks` 가 그렇게 붙는다: 그 목록은
# 최대 500개라 안 그리는 클라에게는 순 비용인데, 그리는 코드와 그 대역폭이 **한
# 디렉토리에 함께** 있어야 지웠을 때 둘 다 사라진다(pytmux-469).
#
