...
def read_capture(path: str) -> bytes:
    """캡처 로그 원시 바이트를 읽는다. `.gz` 면 투명하게 gzip 해제한다(용량 절감으로
    닫힌 로그를 .log.gz 로 보관하므로 — REC_SCENARIO). 일반 `.log` 는 그대로 읽는다."""
    if path.endswith(".gz"):
...
def run_replay(path: str, cols: int, rows: int, ruler: bool = False,
    ...
    try:
        data = read_capture(path)        # .gz 투명 해제
    except OSError as e:
...
def run_record(path: str, cols: int, rows: int, argv: list[str],
    ...
    # Windows 에는 대응물이 없어(후순위, docs/internal/WINDOWS_PORT.md §6-3) 지원하지 않는다.
