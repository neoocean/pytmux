...
class Server(*_SERVER_BASES):
    def __init__(self, sock_path: str, resume_path: str | None = None):
        ...
        # 패널 출력 캡처(REC)는 plugins/rec/ 로 추출됐다(docs/internal/REC_SCENARIO.md). 상태
    ...
    def set_vt_parser(self, value):
        ...
        만들어지므로(model.Pane), 다음 작업 보존 재시작이나 respawn 에서 새 백엔드를
        채택한다. 라이브 패널 거동은 즉시 바뀌지 않는다. docs/VT_PARSER_TRADEOFF §8."""
        if value not in ("pyte", "native"):
            return self.vt_parser
        self.vt_parser = value
        self._save_opts()
        return self.vt_parser
    ...
    def set_plugin_enabled(self, name, on=None):
        ...
        반환: 토글 후 활성 여부(True=켜짐). docs/internal/PLUGIN_MANAGER_SCENARIO.md."""
        if not name:
            return None
    ...
    def handle_control(self, line: str):
        ...
        if c in ("new-session", "new"):
            ...
            # VT 파서 백엔드 선택(pyte|native) — on/off 가 아니라 문자열 인자.
            # 재시작 시 발효. 결과 백엔드명을 반환(잘못된 값이면 현행 유지).
            return self.set_vt_parser(args[0].lower() if args else "")
            ...
            # 전체 재시작(외부 CLI 트리거): 서버 work-preserving re-exec + 연결된
            # 클라가 새 코드로 relaunch 하도록 통지에 relaunch 플래그를 싣는다.
            # 클라-측 :restart-all(begin_restart("all"))과 동일 효과를 외부에서
            # 트리거 — `pytmux cmd restart-all`. 클라 relaunch 는 각 클라가 끊김
            # 처리에서 os.execv 로 수행(client.py). 클라 없으면 서버만 재시작.
            return ("restarting" if self.restart_server(relaunch_clients=True)
                    else "unsupported")
...
def run_server(sock_path: str, resume_path: str | None = None):
    ...
    # 프로덕션 데몬에서만 콜드 스타트 첫 패널 선(先)spawn 을 켠다(serve 가 이 플래그를
    ...
    # 스스로 no-op. 테스트는 run_server 를 거치지 않아 풀이 꺼진 채라 백그라운드로
    # 실제 의사콘솔을 만들지 않는다.
    try:
        from . import conpty
        conpty.enable_pool()
