...
런을 정본의 셀 격자에 얹는다 — 얹는 일 자체는 세 오버레이 공통이라
`clientrender.paint_runs` 로 접었다(2026-08-02g). 여기 남는 것은 **달력만의 것**이다:
어느 패널을 덮나 · 뒤를 어떻게 흐리게 하나 · 클릭존을 코어 표기로 옮기기.
...
def draw_calendar_overlay(cells, panes, calendar_panes, W, H, theme, now=None,
    ...
    runs, zones = calendar_cells(on, {pid: {"offset": off}
                                 ...
                                 now=now)
    paint_runs(cells, runs, W, H, theme)
    # 클릭존은 코어 마우스 핸들러의 표기로 옮긴다(delta = 이 클릭이 옮길 달 수).
