...
def _safe(s) -> str:
    ...
    경로 탈출은 화이트리스트가 이미 막는다(구분자·NUL·ESC 가 통째로 제거된다 — 검수
    새 배터리 2026-07-25 로 재확인). 다만 **점만 남는 잔재**는 따로 정리한다:
    `../../etc/passwd` 는 종전에 `....etcpasswd` 가 됐는데 ① Windows 는 파일명 끝
    점을 잘라 서로 다른 이름이 **충돌**할 수 있고 ② POSIX 에선 선행 점이 숨김 파일을
    만든다. 점 런은 하나로 줄이고 양끝 점은 떼어 낸다(정상 이름 `my.tab` 은 불변)."""
...
class ServerRecMixin:
    ...
    # 단일 캡처 파일 상한(요청 2026-06-16): 초과하면 새 파일로 로테이션해 한 로그가
    # 수백 MB로 폭주(실측 318MB)하는 것을 막는다. 닫힌 세그먼트는 오프라인에서
    # gzip 압축 보관(REC_SCENARIO §11, scripts/compress_captures.py — VT raw 는 ~20-73×).
    _CAPTURE_CAP = 50 * 1024 * 1024

    def _capture_filename(self, pane: Pane, rot: int = 0) -> str:
        """REC 캡처 파일명: <날짜>_<시간>_<세션>_<탭>[-rot]_p<패널>.log (기록 시작 시각).
        예: 20260608_184500_0_0.claude_p1.log. rot>0 은 같은 초 내 로테이션 충돌 회피용."""
        sess, tab, pn = self._pane_idents(pane)
        ts = time.strftime("%Y%m%d_%H%M%S")
        rs = f"-{rot}" if rot else ""
        return f"{ts}_{sess}_{tab}{rs}_p{pn}.log"

    def _capture_open(self, pane: Pane):
        """패널용 새 캡처 파일을 연다(초기 진입·로테이션 공용). 충돌 없는 이름으로
        0600 생성하고 sessions.log 메타를 남긴다. 핸들 반환(실패 시 None)."""
        try:
            os.makedirs(self.capture_dir, exist_ok=True)
            # 캡처 디렉터리는 0700(F4): raw 출력엔 표시·에코된 비밀번호·토큰이 남을 수
            # 있어 같은 머신의 다른 로컬 사용자가 못 읽게 좁힌다(best-effort).
            try:
                os.chmod(self.capture_dir, 0o700)
            except OSError:
                pass
            rot = 0
            while True:                       # 같은 초 로테이션 시 이름 충돌 회피
                path = os.path.join(self.capture_dir,
                                    self._capture_filename(pane, rot))
    ...
    def _capture_write(self, pane: Pane, data: bytes):
        """패널 raw PTY 출력을 캡처 파일에 무손실 append. 현재 파일이 _CAPTURE_CAP 를
        넘기 직전이면 새 파일로 로테이션한다(런어웨이 방지)."""
        fh = self._capfiles.get(pane.id)
        if fh is None:
            fh = self._capture_open(pane)
        elif self._capsizes.get(pane.id, 0) + len(data) > self._CAPTURE_CAP:
            self._close_capfile(pane.id)      # 상한 초과 → 세그먼트 회전
            fh = self._capture_open(pane)
        if fh is None:
            return
        try:
            fh.write(data)
            self._capsizes[pane.id] = self._capsizes.get(pane.id, 0) + len(data)
        except OSError:
            self._close_capfile(pane.id)

    def _close_capfile(self, pane_id: int):
        self._cappaths.pop(pane_id, None)   # 다음에 켤 때 새 시각 파일명으로 재오픈
        self._capsizes.pop(pane_id, None)
