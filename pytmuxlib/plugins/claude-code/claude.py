...
#: 마커 줄의 **첫 글자** 전수 — 위 정규식이 열 0 에 못 박아 둔 것과 같은 집합이다.
#:
#: 스크롤백을 훑는 쪽(`promptblocks`)이 「첫 칸만 보고 후보를 거르는」 축소에 쓴다.
#: 1만 줄을 통째로 문자열로 만들어 정규식을 거는 것은 실측 0.45초라 프레임마다 못 한다 —
#: 첫 칸만 보면 4.4ms 다. **판정 자체는 그래도 위 정규식 하나**이고, 이 표는 그 정규식이
#: 절대 매치할 수 없는 줄을 미리 버리는 것뿐이다. 둘이 갈라지면 블록 경계가 프롬프트
#: 점프와 조용히 어긋나므로 시험이 둘을 대조한다(`test_claude_prompt_blocks`).
PROMPT_MARK_FIRST = (">", "❯")
