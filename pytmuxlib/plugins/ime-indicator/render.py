...
def draw_ime_indicator(cells, W, H, label, st, y=0, reserve_right=4,
    ...
    배치(2026-06-11 사용자 요청으로 우상단 고정 → 커서 줄로 변경; 2026-06-16 →
    활성 패널 우측 끝으로 변경): 기본은 **커서가 있는 줄**(호출부가 y 로 전달)의
    오른쪽 끝 — 조합(preedit)이 보이는 커서 줄과 같은 높이라 한/영 상태를 시선 이동
    없이 확인한다. 오른쪽 경계는 `x_right`(exclusive, 미지정 시 화면 폭 W) — 좌우
    분할에서 활성 패널이 화면 절반만 차지할 때 호출부가 그 패널의 우측 경계를
    넘겨 배지가 활성 패널 안에 그려지게 한다. `reserve_right` 만큼 그 경계에서 더
    비운다 — y=0(첫 행)에서 탭 닫기 `[x]`(콘텐츠 우상단, client_render 뒤에 그려짐)와
    겹치지 않게 하기 위함이고, 다른 행엔 [x] 가 없어 호출부가 0 을 넘겨 진짜 오른쪽
    끝까지 쓴다. 행 오른쪽 끝이 패널 테두리(│)면 그 위에 덮는다 — 상단 테두리에
    그리던 기존과 같은 '의도된 오버레이'(_ime_zone 으로 테두리 강조 검사가 예외 처리).
    ...
    text = "[" + label + "]"
    w = _text_width(text)
    right = W if x_right is None else x_right
    x_end = right - reserve_right     # 배지가 차지할 오른쪽 경계(exclusive)
    x0 = x_end - w
    if x0 < 0:
