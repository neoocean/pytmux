...
def _find_chain(root: str, target: str) -> list[list]:
    """root→target 조상 사슬을 [경로, 직계하위] 목록으로(root 포함, target 미포함).
    클라가 이 사슬로 _children 를 채우고 각 조상을 펼치면 target 행이 보이게 된다."""
    root = os.path.normpath(root)
    parts: list[str] = []
    p = target
    for _ in range(64):                         # 깊이 상한(방어)
        parts.append(p)
        if os.path.normpath(p) == root:
            break
        parent = os.path.dirname(p.rstrip("/\\"))
        if not parent or parent == p:
            break
        p = parent
    parts.reverse()                              # root..target
    return [[a, _list_dirs(a)] for a in parts[:-1]]   # target 의 부모까지(target 은 잎)


def nc_find_msg(server, sess, query: str, root: str | None = None) -> dict:
    """`query` 와 이름이 맞는 디렉토리를 **트리에 열려 있지 않은 곳까지** 재귀로 찾아
    (요청 2026-06-16) 최적 매치 경로 + 그 조상 사슬을 회신한다(클라가 펼쳐 선택).

    경계(대형/네트워크 트리 방어): 스캔 디렉토리 수·깊이 상한, 숨김(`.`) 제외,
    심링크 미추적. 우선순위: 접두 일치 > 부분 일치, 같으면 얕고 짧은 경로. 못 찾으면
    target=None. NCD 클래식의 '전체 트리 점프'를 지연 로드 트리 위에서 근사한다."""
    from collections import deque
    q = (query or "").strip().lower()
    if not q:
        return {"t": "nc_found", "query": query, "target": None, "chain": []}
    base = os.path.abspath(os.path.expanduser(root)) if root else os.path.abspath(os.sep)
    if not os.path.isdir(base):
        base = os.path.abspath(os.sep)
    LIMIT, MAXDEPTH = 20000, 12
    best = None                                  # (score, path); score 작을수록 우선
    scanned = 0
    dq = deque([(base, 0)])
        ...
        try:
            with os.scandir(cur) as it:
                for e in it:
                    ...
                    nm = e.name.lower()
                    if q in nm:
                        score = (0 if nm.startswith(q) else 1, depth, len(e.path))
    ...
    if best is None:
        return {"t": "nc_found", "query": query, "target": None, "chain": []}
    target = best[1]
    return {"t": "nc_found", "query": query, "target": target,
            "chain": _find_chain(base, target)}
