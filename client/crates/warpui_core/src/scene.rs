...
pub struct Layer {
    ...
    /// Source text recorded alongside the glyphs (see [`Scene::record_text`]).
    /// Glyph ids cannot be mapped back to characters without the font, so
    /// text-painting elements record what they painted here — this is what makes
    /// a headless "what is visible" oracle possible (rasterization ignores it).
...
/// The source string a text-painting element drew, with the bounds it was
/// painted into. Recorded via [`Scene::record_text`]; queried with
/// [`Scene::painted_texts`].
#[derive(Clone, Debug)]
...
impl Scene {
    ...
    /// Records the source string of painted text on the active layer.
    ///
    /// Glyphs carry only glyph ids, which cannot be mapped back to characters
    ...
    /// All recorded text in paint order (bottom layer first). This answers
    /// "what is visible on screen" in headless tests, which glyph ids alone
    /// cannot.
    pub fn painted_texts(&self) -> impl Iterator<Item = &PaintedText> {
        self.layers().flat_map(|layer| layer.texts.iter())
    }
