    ...
    fn dispatch_event(
        ...
        match event.raw_event() {
            ...
            Event::LeftMouseDragged { position, .. } => {
                // ★ 끄는 동안에도 **"마우스가 내 위에 있나"는 참이라야 한다**(2026-07-31,
                //   PROVENANCE §1 — pytmux-gui 탭 드래그). 종전에는 이 갱신이
                //   `MouseMoved` 에만 있어서, 버튼을 누른 순간부터 모든 `Hoverable` 의
                //   `is_mouse_over_element` 가 **누른 그 순간 값에 얼어붙었다**. 드롭 대상을
                //   hover 로 알아내는 쪽(탭 드래그)은 그래서 대상을 영영 못 찾았다.
                //
                //   hover **핸들러·타이머는 일부러 안 돌린다** — 저것들은 "머무름"의 뜻이고
                //   드래그 중의 통과는 머무름이 아니다. 여기서 고치는 것은 **사실 한 칸**뿐이다.
                let is_hovered = self.is_mouse_over_element(ctx, *position);
                if self.state().is_mouse_over_element != is_hovered {
                    self.state().is_mouse_over_element = is_hovered;
                    ctx.notify();
                }
