...
pub static EN: &[(&str, &str)] = &[
    ...
    // ── proto/diag.rs — `debug-stats` 판의 **서버 절반**(pytmux-382) ──
    ("서버 쪽", "Server"),
    ("기동 뒤", "up"),
    ("스레드", "threads"),
    ("asyncio 태스크", "asyncio tasks"),
    ("클라", "Clients"),
    ("마지막 수신", "last seen"),
    ("원격 링크", "remote links"),
    ("재연결 중", "reconnecting"),
    // 「세션」은 en_proto 가 이미 든다(중복 키 게이트) — 그쪽 번역을 그대로 쓴다.
    ("스크롤백 행", "scrollback rows"),
    ("산 객체", "Live objects"),
    ("수거", "collections"),
    ("거둔 것", "collected"),
    ("못 거둔 것", "uncollectable"),
    ("프로브 마지막 회차", "probe last run"),
    ("성공", "ok"),
    ("실패", "failed"),
    ("프로브가 아직 한 번도 안 돌았다", "probe has not run yet"),
    ("전", "ago"),
