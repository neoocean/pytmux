...
static NATIVE_PLUGIN_COMMANDS: &[&str] = &[
    ...
    "open-clock",
    "prompt-clear",
    // ★ **캔버스 위의 고르기**(pytmux-18 · 정본은 pytmux-469 로 뒤따라 섰다). 왜 서버가
    //   못 하나: 고른 자리는 **그 클라만 아는 상태**다 — 같은 패널을 두 클라가 보고 있을
    //   때 한쪽이 고른 블록이 다른 쪽 화면에서 밝아지면 안 된다(설계 §4.4 `client_fact`,
    //   위 오버레이 넷과 같은 근거). 서버가 주는 것은 경계(`blocks` 메시지)와 그 범위의
    //   글(`copy_range`)까지이고, 고르기·강조·복사 손동작은 양쪽 클라가 각자 든다.
    "select-blocks",
