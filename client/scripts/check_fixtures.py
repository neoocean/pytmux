...
생성기 열여섯은 정본을 직접 import 해 픽스처를 뽑고, Rust 적합성 테스트가 그 픽스처를
...
그래서 바이트 동일은 **아무도 지켜 주지 않는 성질**이었고, 실제로 셋이 겹쳐 터졌다:
 ...
 1. **CI**: 생성기(그때 열여덟)는 플랫폼 기본으로 쓰고 하나만 `newline="\n"` 로 LF 를 써서
    Windows 러너에서만 붉었다(2026-08-01, p4 69128 이 그 하나를 나머지에 맞춰 껐다).
