...
> 기준 문서는 **`docs/reports/2026-08-01-ui-parity-side-by-side.md`**(3차 대조 · 20장면)
...
사용법(키·마우스·화면·설정 — 실물 GUI 스크린샷 포함)은 **`docs/USER_GUIDE.md`** 다.
...
- `scripts/check_fixtures.py` — **픽스처가 정본보다 낡지 않았나.** 생성기 열여덟을 전부
...
→ 게이트 → `PYTMUX_HOME` 격리 서버에 **라이브** → `docs/reports/` 에 리포트 한 장 → CL 하나.
...
- ⚠ **좌표계**: 캡처 그림은 **물리 픽셀**(DWM 확장 프레임 기준)인데 앱이 보는 마우스
  좌표는 **클라이언트 영역의 논리 픽셀**이다. 이 상자는 150% 라 **그림 좌표 ÷ 1.5 를
  `-ClientPixels`** 로 줘야 앱이 보는 자리와 같아진다. 비율 모드로 그림에서 고른 자리를
  그대로 주면 탭·×·경계를 **전부 빗나가고**, 증상은 "hover 도 클릭도 죽었다"로 보인다
  (2026-07-30: 그렇게 읽고 "크롬 마우스가 통째로 죽었다"고 결론 낼 뻔했다 — 앱을
  의심하기 전에 **임시 `log::warn!` 로 앱이 받은 좌표를 찍어** 보정할 것).
...
- 이 상자는 **150% 배율**이라 스크린샷 픽셀이 논리 좌표의 1.5배다.
