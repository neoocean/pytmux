...
| `crates/warpui_core/src/scene.rs` + `elements/gui/{text,shimmering_text}.rs` | Scene 에 그려진 글자의 **원문 기록** 추가(`Layer.texts` · `Scene::record_text`/`painted_texts`) — 글자를 그리는 엘리먼트가 페인트할 때 원문·bounds·**색**을 같이 남긴다 | Scene 의 글리프는 glyph_id 라 글자로 못 되돌린다 — 그래서 "화면에 무엇이 보이나"를 헤드리스로 잴 길이 없었고, GUI 그리기 배선 누락(G8p 류)은 라이브 스크린샷만 잡았다. 이 기록이 그리기 오라클(클라 g9aa)을 가능하게 한다. 래스터라이저는 이 필드를 안 읽는다(부작용 0). ★ **색을 함께 남기는 것은 2026-08-24 에 더했다**(pytmux-376) — pytmux 의 크롬에는 «색만으로 말하는» 자리가 여럿이라(작업이 끝난 탭·모드 표식·흐린 꼬리줄) 글자만 재는 오라클은 «그려졌지만 엉뚱한 색»과 «제대로 그려졌다»를 못 가른다. `record_text` 는 **우리가 더한 함수**라 상류 호출부가 없고, 부르는 자리는 이 저장소의 둘뿐이다(그래서 §45 의 «시그니처를 바꾸면 상류 테스트가 깨진다» 가 여기엔 없다) |
