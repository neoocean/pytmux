...
```sh
ls -la client/build/          # 파일·크기·날짜
```

수를 글자로 적어 두면 낡는다 — 실제로 낡았다. 2026-08-01 에 *"지금 실물은 macOS 것
하나뿐이다"* 라고 적어 뒀는데(그때는 참이었다: Linux 것은 없었고 Windows 것은 손으로
구워 실 경로를 품고 있어 지웠다), 그 사이 셋이 다 채워졌고 문장만 남았다 — 배포 축에서
가장 먼저 읽히는 문단이 **1년 중 가장 틀리기 쉬운 자리**였다(검수 2026-08-09 B-6 ·
그때 실측: macOS 7.6MB(08-01) · Linux 29MB(08-01) · Windows 16.9MB(08-09 CL 71347)).

위 표는 "CI 가 채울 자리"의 정의이기도 하다 — 셋이 다 안 오면 publish 잡이 실패한다.
