...
⚠️ **이 하네스는 신뢰성이 부족하다(2026-06-11 재검증)**: 자기 콘솔을 가진 진단 프로세스
(`Start-Process -WindowStyle Hidden`)에선 배너+echo 가 도달해 [1][2] 가 PASS 처럼 보여도,
**정작 제품이 도는 콘솔-less 데몬에선 owned 가 자식 출력을 전혀 스트리밍하지 못한다**(패널
백지). 즉 이 스크립트의 배너+echo 체크는 그 **데몬 스트리밍 갭을 못 잡는다**. owned 는 현재
**비동작**이며(§1.1 ⚠️ 미해결 재확정), 충실한 검증은 **데몬 경유 라이브 테스트**(run-pytmux
드라이버로 실제 서버 패널 렌더 확인)여야 한다. 상세: docs/IMPROVEMENT_OPPORTUNITIES.md §1.1.
