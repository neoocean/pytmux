...
LEAK_PATTERNS = (
    ...
    # 테일스케일(CGNAT `100.64/10` 대역) 주소 — **사설망의 위치**다(검수 2026-09-05 C-1 ·
    # 사용자 결정 2026-09-05: "주소를 가리고 게이트도 세운다"). 실제로 이 저장소의
    # `CLAUDE.md`·`qa/README.md` 에 트래커 웹·**무인증** MCP JSON-RPC·p4 서버의 주소가
    # 들어가 있었고, 그 둘은 gitignore 밖이라 공개 미러가 그대로 읽는다. 100.x 는 인터넷
    # 라우팅이 안 되지만 「어디에 무엇이 떠 있는지」는 그 자체로 값이다.
    # ⛔ 허용 목록을 두지 않는다 — 지금 주소는 p4 전용 `docs/internal/TRACKER_ENDPOINT.md`
    # 로 옮겼고, 예외를 하나 파 두면 다음 주소가 그 구멍으로 따라 들어온다.
    # 문서용 주소(RFC 5737: `192.0.2.` · `198.51.100.` · `203.0.113.`)는 이 범위 밖이라
    # 안 걸린다 — 시험이 「루프백이 아닌 것」을 적어야 할 때 쓸 자리가 남아 있다.
    (re.compile(r"\b100\.(?:6[4-9]|[7-9]\d|1[01]\d|12[0-7])"
                r"\.\d{1,3}\.\d{1,3}\b"),
     "테일스케일(CGNAT) 주소 — 사설망 호스트의 위치"),
)
