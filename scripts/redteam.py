...
#
# **Windows 는 재는 것이 다르다** — POSIX 는 `/proc/<pid>/fd`(정확히 열린 fd)인데
# Windows 는 `GetProcessHandleCount`(스레드·이벤트·파일까지 **전 커널 핸들**)다. 그
# 상시 배경이 임계를 넘나들어 GHA windows 가 오래 붉었다(관측: 연결 26개 배터리 Δ17~18,
# 연결 1200개 배터리 Δ21 — **연결 수에 안 비례하는 고정 잡음 ~20**이고, 3s 정착을
# 기다려도 안 내려간다). 16 은 POSIX 계측의 값이라 Windows 에 그대로 쓰면 계측 잡음을
# 결함으로 읽는다.
#
# 그래서 Windows 만 64 로 둔다 — 관측 잡음의 3배. 이 게이트가 잡으려는 것은 **연결당
# 미반환**이고 CLI 배터리는 연결이 1200개라 진짜 누수의 신호는 세 자리다(64 대비 19배
# 여유). 다만 in-test 배터리(연결 26개)는 Windows 에서 신호(≈26)가 이 잡음(≈20)에
# 묻혀 **판별력이 없다** — 그쪽 계약은 POSIX 계측과 CLI 규모가 지킨다(test_redteam
# 의 해당 테스트 주석에도 적어 뒀다).
_FD_SLACK = 64 if ipc.IS_WINDOWS else 16
