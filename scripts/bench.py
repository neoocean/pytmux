#!/usr/bin/env python3
"""pytmux 벤치마크 — OS별 성능/반응성 측정 (docs/internal/WINDOWS_TESTING.md / HANDOFF §10).
...
측정 3축:
  ...
  3. **output_flood** — 터미널 출력 폭증 시 처리량(feed MB/s)과 반응성(슬라이스 지연
     p50/p99/max). scripts/poc/feed_profile.py 의 합성 워크로드(claude busy 풀리페인트 / plain
     cat 스크롤)를 재사용한다.
...
def render_markdown(d: dict) -> str:
    ...
    L.append("> 헤드리스 측정(실 셸/ssh 미포함). 인터랙티브 ssh 반응성은 실 박스 필요 — "
             "docs/internal/WINDOWS_TESTING.md §3-c 참고.")
    L.append("")
    return "\n".join(L)
