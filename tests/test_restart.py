"""작업 보존 서버 재시작(re-exec) 테스트 — docs/internal/RESTART_SCENARIO.md §7.
...
async def test_client_reconnects_on_restarting():
    ...
    않고 같은 소켓 경로로 재접속한다(docs/internal/RESTART_SCENARIO.md ⓔ). 실 execv 대신
...
# ── 10. 외부 CLI restart-all: relaunch 통지 → 클라가 relaunch(in-place 재접속 아님) ──
async def test_restarting_with_relaunch_flag_sets_relaunch_mode():
    """서버가 {"t":"restarting","relaunch":true} 를 보내면(외부 CLI restart-all),
    클라가 in-place 재접속 대신 relaunch 모드로 들어간다 — 끊김 처리에서 _relaunch
    를 세워 os.execv(새 클라 코드)하도록 _relaunch_on_restart 가 켜진다. 기존
    ...
    try:
        async with app.run_test(size=(100, 30)) as pilot:
            ...
            # relaunch 없는 통지 → in-place 재접속(relaunch 모드 안 켜짐)
            app._dispatch({"t": "restarting"})
            assert app._reconnecting is True
            assert app._relaunch_on_restart is False, "relaunch 없는 통지는 in-place"
            # relaunch 통지 → relaunch 모드
            app._dispatch({"t": "restarting", "relaunch": True})
            assert app._relaunch_on_restart is True, "relaunch 통지로 relaunch 모드"
    finally:
        await teardown(srv, task, sock)


async def test_control_restart_all_triggers_relaunch_restart():
    """외부 제어 `restart-all`(별칭 full-restart/restart-client-server)이
    restart_server(relaunch_clients=True) 를 부르고 "restarting" 을 반환한다.
    실제 execv 는 테스트 프로세스를 갈아끼우므로 restart_server 를 가로채 인자만 검증."""
    srv, task, sock = await server_only()
    try:
        srv.ensure_default_session(80, 24)
        calls = []
        srv.restart_server = lambda relaunch_clients=False: (
            calls.append(relaunch_clients) or True)
        for name in ("restart-all", "full-restart", "restart-client-server"):
            calls.clear()
            assert srv.handle_control(name) == "restarting", name
            assert calls == [True], (name, calls)
        # restart-server(별칭 restart)는 relaunch 안 함(in-place 재접속).
        calls.clear()
        assert srv.handle_control("restart-server") == "restarting"
        assert calls == [False], calls
    finally:
        await teardown(srv, task, sock)


async def test_restart_server_relaunch_flag_on_wire():
    """restart_server(relaunch_clients=True) 가 클라에 보내는 재시작 통지에 실제로
    ...
    def _decode_frames(buf):
        ...
        while off + 4 <= len(buf):
            ...
            if off + length > len(buf):
                break
            msgs.append(_json.loads(buf[off:off + length]))
            off += length
        return msgs
    ...
    async def _run(relaunch):
        srv, task, sock = await server_only()
        try:
            srv.ensure_default_session(80, 24)
            srv._do_execv = lambda argv: None       # execv 무력화(프로세스 보존)
            w = _CapWriter()
            srv.clients.append(ClientConn(w))
            assert srv.restart_server(relaunch_clients=relaunch) is True
            await asyncio.sleep(0.05)                # create_task(write_msg) 소화
            # 프레임을 디코드해 restarting 통지를 찾는다.
            msgs = _decode_frames(b"".join(w.frames))
            restarting = [m for m in msgs if m.get("t") == "restarting"]
            assert restarting, msgs
            return restarting[0]
        finally:
            await teardown(srv, task, sock)

    m_on = await _run(True)
    assert m_on.get("relaunch") is True, m_on
    m_off = await _run(False)
    assert "relaunch" not in m_off, m_off
