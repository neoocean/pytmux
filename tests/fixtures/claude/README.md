...
## `banner_mascot.ansi` — **날 SGR** 픽스처 (2026-08-16 · pytmux-205)

⛔ 위의 `.txt` 들과 **성격이 다르다**. 저것들은 이미 렌더된 *화면 글자*라 색이 없다.
이것은 Claude Code 시작 배너 3줄의 **날 바이트**(ESC 시퀀스 포함)이고, 그래서 색이
어디서 틀어지는가를 재는 데 쓴다(`tests/test_claude_banner_colors.py`).

- 출처: 이 맥에서 Claude Code **2.1.233** 을 pty 로 띄워 뜬 실제 배너
  (`FORCE_COLOR=3` · `COLORTERM=truecolor` · 120×40). **2.1.229** 도 같은 SGR 을 낸다.
- 홈 경로만 `~/src/example` 로 바꿨다(공개 미러). 나머지는 한 바이트도 안 고쳤다.
- ⛔ **파일 타입은 `binary` 다.** 본문에 **날 `\r`** 이 들어 있어서(정본이 줄을 그렇게
  넘긴다) text 로 두면 클라이언트 줄끝 변환이 그 바이트를 바꾼다 — 그러면 배너가
  한 줄씩 밀린 **다른 화면**이 된다. 읽는 쪽도 반드시 `open(..., "rb")` 다.
- ★ 세 줄의 SGR 구조가 서로 다르고 **그 차이를 일부러 남겼다**: 1번 줄만 마스코트와
  회색 글자 사이에 `ESC[39m` 이 있고 2·3번 줄은 truecolor → truecolor 로 곧장 간다.
  제보(pytmux-205)에서 색이 틀어진 것이 **정확히 그 두 줄**이라, 손으로 다듬으면
  이 픽스처의 값어치가 사라진다.
