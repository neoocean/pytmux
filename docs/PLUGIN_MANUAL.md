...
지금 저장소에 있는 플러그인(11개 — 화면은 [GALLERY.md](GALLERY.md) §7 참고):
...
| `claude-resume` | 이 머신의 Claude 세션 목록 → 골라 새 탭에서 리줌 | 모달 + 서버 왕복 | 중간 | — |
| `rec` | 패널 raw PTY 출력 무손실 캡처(REC) | 서버 믹스인 + status 흡수 | 작음 | — |
...
| `claude-disable-feedback` | Claude 피드백 권유 문구(시작 팁·세션 종료 평가 배너) 숨김 | 서버 렌더 필터(`server_filter_rows`) | 작음(단일 훅) | [§13.1](#131-분리된-예--claude-disable-feedback) |
...
둡니다. 명령만 21개(시작 규칙·토큰 절감·자동 재개·토큰 로그·권한모드·자동 문서화 등).
...
### 13.1 분리된 예 — `claude-disable-feedback`

`claude-code` 가 모든 축을 쓰는 거인이라면, `claude-disable-feedback` 은 그 반대편 —
**단일 훅 하나**로 끝나는 가장 작은 플러그인입니다. Claude Code 가 띄우는 두 가지 피드백
권유 문구를 화면에서 가립니다: 시작 팁 `Tip: Use /feedback to help us improve!` 와 세션
종료 평가 배너 `How is Claude doing this session?`(`1: Bad … 0: Dismiss`). 원래 `claude-code`
안에 있었지만, 끄고 켤 수 있는 독립 기능이라 2026-06 자기 디렉토리로 분리했습니다.

핵심은 `server_filter_rows` 한 훅뿐입니다. 코어는 render 된 행 목록(행 = `[text, style]`
런 목록)을 **클라 전송 직전에** 활성 플러그인들에 차례로 흘려 보내고, 각 플러그인은 행을
변형(또는 그대로 통과)할 수 있습니다([§4.3](#43-서버-훅-메서드)). 이 플러그인은 피드백
문구가 든 줄만 **폭을 유지한 채 공백 런으로** 바꿔 화면에서 지웁니다(레이아웃 불변).

```python
class _ClaudeDisableFeedbackPlugin:
    name = "claude-disable-feedback"
    def server_filter_rows(self, server, pane, rows):
        # Claude 패널만 — 게이트는 claude-code 가 패널에 단 속성을 getattr 로 부드럽게 참조.
        if not (getattr(pane, "_claude", None) or getattr(pane, "_hdr_claude", None)):
            return rows
        rows = _blank_feedback_tip(rows)       # 시작 팁 줄 → 공백
        rows = _blank_feedback_banner(rows)    # 세션 종료 배너 두 줄 → 공백
        return rows
```

이 작은 플러그인이 보여 주는 세 가지 패턴:

- **훅 체인**: 레지스트리의 `server_filter_rows` 는 활성 플러그인을 순회하며 앞 결과를
  다음에 넘깁니다 — 그래서 `claude-disable-feedback` 은 `claude-code` 를 건드리지 않고
  같은 행 흐름에 자기 변형을 **덧붙입니다**(둘 다 활성이면 순차 적용).
- **플러그인 간 느슨한 결합**: 게이트로 쓰는 `_claude`/`_hdr_claude` 는 `claude-code` 가
  패널에 설치한 속성입니다. 하드 import 가 아니라 `getattr(..., None)` 으로 참조하므로,
  `claude-code` 가 없으면 둘 다 없어 모든 패널이 그냥 통과(no-op)합니다 — [§6.2](#62-인스턴스-글루-attach_client)
  의 "플러그인끼리도 하드 의존 금지" 규약과 같습니다.
- **표시 필터 ≠ 키 주입**: 예전엔 배너를 `Esc` 자동 주입으로 닫으려 했지만, 단일 Esc 가
  종종 Dismiss 대신 **작동 중인 턴을 interrupt** 했습니다(배너는 컴포저 위 비모달이라 안
  닫아도 작업을 막지 않음). 그래서 키를 쏘지 않고 **렌더 단계에서 표시만 가리는** 쪽이
  안전합니다 — busy 턴과 레이스하지 않습니다.

`rm -rf pytmuxlib/plugins/claude-disable-feedback/` 하면 두 문구가 다시 보일 뿐, 코어와
`claude-code` 는 그대로 동작합니다(delete-to-disable). 가역적으로 끄려면 플러그인 관리
팝업([§2.1](#21-지우지-않고-끄기--플러그인-관리-팝업))에서 토글하면 됩니다.
