...
> - 설계 배경·추출 진행 기록(특히 대형 `claude-code` 추출)은 [PLUGIN_SYSTEM.md](PLUGIN_SYSTEM.md).
> - 사용자용 전체 매뉴얼은 [MANUAL.md](MANUAL.md), 내부 설계는 [DESIGN.md](DESIGN.md).
> - 스크린샷은 모두 `scripts/gen_screenshots.py` 가 **실제 클라이언트를 헤드리스로 운전**해
>   생성합니다([SCREENSHOT_SCENARIO.md](SCREENSHOT_SCENARIO.md)).
...
9. [무게 규칙: 서버도 같은 코드를 읽는다](#9-무게-규칙-서버도-같은-코드를-읽는다)
10. [테스트와 계약 검증](#10-테스트와-계약-검증)
11. [체크리스트와 함정](#11-체크리스트와-함정)
...
| 플러그인 | 무엇 | 규모 | 참고 |
|---------|------|------|------|
| `clock` | 패널을 큰 블록 시계로 덮는 오버레이 | 작음(레퍼런스) | [§6](#6-사례-1--시계-플러그인-clock) |
| `calendar` | 패널을 이번 달 달력으로 덮는 오버레이 | 작음(`clock` 미러) | [§7](#7-사례-2--달력-플러그인-calendar) |
| `ncd` | Norton Change Directory 풍 디렉토리 트리 | 중간 | [NC_SCENARIO.md](NC_SCENARIO.md) |
| `claude-code` | Claude Code 통합(스캔·상태·토큰·렌더 전체) | 큼(ncd 의 ~10배) | [PLUGIN_SYSTEM.md](PLUGIN_SYSTEM.md) §4 |
...
됩니다 — [§9 무게 규칙](#9-무게-규칙-서버도-같은-코드를-읽는다)이 여기서 나옵니다.
...
2. **`__init__.py` 최상단에서 textual/rich 를 import 하지 않는다**([§9](#9-무게-규칙-서버도-같은-코드를-읽는다)).
...
기능이 한 디렉토리 안에 있되 **import 무게**로 셋을 가릅니다(§9 무게 규칙의 모범):
...
파일을 읽기 때문입니다(§9). `screen.py` 의 textual 위젯은 `_on_nc_list` 안에서, `server.py`
