...
> 재시작이 필요합니다. 자세한 동선은 [`PLUGIN_MANAGER_SCENARIO.md`](PLUGIN_MANAGER_SCENARIO.md) §5.
...
- **휴리스틱은 폴백**: OS 실측이 안 되는 환경(ssh 원격·리눅스·Windows)에서만 `client_key`
...
큰 규모라 본 매뉴얼은 **개요**만 두고, 단계적 추출 기록·모듈 지도·서버 믹스인 합성 방식은
[PLUGIN_SYSTEM.md §4](PLUGIN_SYSTEM.md) 에 정리돼 있습니다. 핵심은 이만한 기능도 **delete-to-
