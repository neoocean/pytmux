...
8. [무게 규칙: 서버도 같은 코드를 읽는다](#8-무게-규칙-서버도-같은-코드를-읽는다)
9. [테스트와 계약 검증](#9-테스트와-계약-검증)
10. [체크리스트와 함정](#10-체크리스트와-함정)
...
됩니다 — [§8 무게 규칙](#8-무게-규칙-서버도-같은-코드를-읽는다)이 여기서 나옵니다.
...
2. **`__init__.py` 최상단에서 textual/rich 를 import 하지 않는다**([§8](#8-무게-규칙-서버도-같은-코드를-읽는다)).
...
### 9.2 delete-to-disable 계약 검증 (`tests/test_plugin_contract.py`)
