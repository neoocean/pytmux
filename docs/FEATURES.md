...
| **토큰 과사용 자동 회피(고유)** | — | `token-saver` 명령으로 **설정 팝업**을 열어 각 자동 개입을 토글: ① 컨텍스트 잔량%<임계 시 자동 정리(`/compact` 또는 doc→/clear, 응답 완료 경계 1회) ② 일/세션 토큰 예산 + 상태줄 ⚠경고 ③ 예산 초과 시 자동재개 보류. 모두 기본 OFF·전역 영속. 안전 게이트: 발화직전 재확인·디바운스·busy 취소. 설계: docs/TOKEN_SAVING_SCENARIO.md | P2 | ✅ |
