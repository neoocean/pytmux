...
| **Claude 마지막 프롬프트 헤더(고유)** | — | Claude 실행 패널 내부 맨 윗줄에 마지막 제출 프롬프트를 다른 색의 스티키 헤더로 표시(스크롤 무관), 우측 `[x]` 로 닫기·새 프롬프트 시 재표시. 타이핑·**붙여넣기(모바일 받아쓰기/자동완성 포함)** 모두 추적 | P2 | ✅ |
...
| **작업 보존 재시작(re-exec)** | tmux 서버 업그레이드 | `restart-server`(별칭 `restart`): 살아 있는 셸/프로그램·PTY 를 보존한 채 서버 코드만 제자리 `os.execv` 로 교체. master fd 상속+CLOEXEC 재채택, 패널/Claude 상태·화면 스냅샷 복원, 클라이언트 자동 재접속. 설계·명세: docs/RESTART_SCENARIO.md | P3 | ✅ |
