...
| **탭 삭제** | `prefix &` (kill-window) | 탭바 `[x]`·`prefix &`·키바인딩 모두 **중앙 확인 팝업**(`닫기`/`취소`, 기본=취소). `kill-tab` | P0 | ✅ |
| **상단 탭바** | (tmux 없음) | 상단 탭바([+]/탭/[x]) **항상 표시(기본)**. 마우스 클릭, ESC 모드 위 방향키로 포커스→←→ 선택→Enter 전환, 폭 초과 시 ◀▶ 스크롤. `set tab-bar auto`(2개↑일 때만) | P1 | ✅ |
...
| **시계 모드** | `prefix t` (clock-mode) | **현재 패널 전체를 큰 시계로 덮는다**. 뒤의 터미널 출력은 흐리게(dim) 계속 갱신되어 보이고, 우상단 `[x]` 클릭·그 패널에서 `clock-mode` 명령·**하단 바 시계 클릭**(토글)으로 닫는다 | P3 | ✅ |
...
| **Claude 마지막 프롬프트 헤더(고유)** | — | Claude 실행 패널 내부 맨 윗줄에 마지막 제출 프롬프트를 다른 색의 스티키 헤더로 표시(스크롤 무관), 우측 `[x]` 로 닫기·새 프롬프트 시 재표시 | P2 | ✅ |
