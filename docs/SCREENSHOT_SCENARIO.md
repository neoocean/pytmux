...
> ① **결정적 장면 29개**(API·네트워크 불필요) — 무인자 `gen_screenshots.py` 가 장면별
> 격리 서브프로세스로 전부 생성(권위 목록은 `scripts/gen_screenshots.py` 의 `SCENES`).
> ② **라이브 Claude 컷 4개**(11/12/13/22) — **진짜
> `claude` CLI 를 패널에서 한 세션 실행**해 처리중(◐)·응답완료·자동재개(AR)·권한모드
> 팝업을 캡처한다(`claude_suite`). 실제 API 호출(인증·토큰)이라
...
2. **장면 카탈로그** — 권위 목록은 `scripts/gen_screenshots.py` 의 `SCENES`(결정적)와
   `CLAUDE_OUTPUTS`(라이브)다. 아래는 현재 등록된 장면(2026-06 기준, 번호는 결번 있음):
   - 본체: `01-first-run`·`02-split-lr`·`03-split-nested`·`04-zoom`·`05-menu`·
     `06-command-prompt`·`07-kill-pane-prompt`·`08-tabs-multi`·`10-confirm-tab`·
     `19-confirm-tab-last`·`14-info-popup`·`15-scrollback`·`16-degraded`·
     `17-command-popup`·`21-restart-check`·`26-restart-confirm`·`34-remote-attach`
   - 플러그인: `18-clock`·`09-calendar`·`27-ncd`·`33-ime`·`31-prompt-history`·
     `30-usage-view`·`32-p4changes` + claude-code 팝업(`23-token-saver`·`24-token-log`·
     `25-remote-control`·`28-claude-rules`·`29-usage-panel`)
   - 라이브 Claude(`claude-suite`): `11-claude`·`12-claude-autoresume`·`13-perm-mode`·
     `22-claude-real`
   - (옛 초안에 있던 `10-claude-header` "스티키 헤더"는 기능 제거로 폐지됨 → 프롬프트
     히스토리는 `31-prompt-history` 플러그인 컷으로 대체)
