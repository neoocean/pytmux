...
| 6 | **A1** textual import 다이어트 | 낮음(현실적) | 낮음 | ☑ 측정·종결(§8.7) |
...
- **A1**(측정·종결): 클라 import ~99ms 중 **textual 코어 ~57ms(css.model 44ms 지배)는
  ...
  - **후속 정리(2026-06-14)**: §5.4 client.py 믹스인 분할(p4 58842~58847)에서 클로저가
    믹스인 메서드로 바뀌며 죽은 import 7개 재유입(AST 점검 적발) → `Input`(textual.widgets)·
    `MENU_ITEMS`/`MENU_TOGGLES`/`SPECIAL`/`_DATE_STRFTIME`/`_TIME_STRFTIME`/`_ONOFF`
    (clientutil noqa 블록, 본문 0회 참조) 제거. textual.widgets import 표면도 1줄 축소.
    621 passed 불변. 이것이 A1 의 유일한 실재 잔여 레버였다(대형 레버는 위 종결대로 부재).
