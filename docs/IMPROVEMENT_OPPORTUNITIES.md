...
### [M] 3.8 Claude Code 스크롤백을 '프롬프트' 단위로 묶어 히스토리로 표시 (요청 2026-06-12) ✅ **완료(Stage 1·2①②③)**
...
### [M] 4.5 `_status_msg` 가 패널마다 `prompt_history[-30:]` 매 프레임 직렬화 — `serverio.py` ✅ **해결(검증 2026-06-12, stale 헤딩 정정)**
> 주기 status(full=False)는 `prompt_history` 가 **변할 때만** history 를 싣는다(serverio.py:301
> `§4.5`, 클라 `_update_claude` 는 빠진 항목에 직전 history 유지). 테스트 `test_server.py` §4.5.
> (아래는 수정 전 진단.)
