...
### [M] 2.6 줌 중 비활성 패널 winsize 정지로 reflow 깨짐 가능 — `model.py:757-761` (추정)
`compute_layout` 이 줌 시 활성 패널만 리스트에 넣어 다른 패널 PTY resize 통지가 멈춤 →
줌 중 도착 출력이 옛 크기로 feed 돼 일반 셸 스크롤백 잘림. **개선**: 줌 중에도 논리
크기 유지 또는 진입/해제 시 전 패널 resize 강제 + 회귀 테스트. **위험**: 중(레이아웃
핫패스).
