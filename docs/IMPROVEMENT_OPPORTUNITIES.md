...
### [M] 3.8 Claude Code 스크롤백을 '프롬프트' 단위로 묶어 히스토리로 표시 (요청 2026-06-12) 🚧 **Stage 1 구현**
...
- **Stage 1 완료**: 프롬프트 제출(두 경로: `_track_prompt`·스캔)마다 anchor 를 prompt_history
  ...
  통합 테스트 4건.
- **Stage 2 후속(UX 결정)**: ① 위로 스크롤 → 오버레이 자동 표시(raw 스크롤 대체 vs 별도
  제스처 — 결정 필요), ② 팝업에서 Enter 직접 선택 점프(InfoScreen on-select 확장), ③ 선택
  '펼치기'(인접 anchor 로 구간 `[anchor_i, anchor_{i+1})` 분리 표시). 시나리오 문서 §Stage 2.
