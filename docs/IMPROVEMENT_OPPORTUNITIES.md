...
> **★ 해결(2026-06-11) — 직접 소유 ConPTY 백엔드 `pytmuxlib/conpty.py` 신설, raw 바이트:**
> 막혔던 원인은 파이프 종류도 ctypes 형식도 아니라 **런처가 콘솔을 상속**한 탓이었다.
> 직전 probe 들은 NonInteractive PowerShell 도구의 리다이렉트된 콘솔 아래에서 돌아, 자식
> cmd.exe 가 우리 의사 콘솔 대신 그 부모 콘솔을 붙잡았다(그래서 배너가 부모 stdout 으로
> 누수·read 단 0 바이트). **런처가 자기 콘솔을 가지면**(데몬 서버는 콘솔 없는 분리
> 프로세스라 자연 충족; probe 는 `Start-Process -WindowStyle Hidden` 로 재현) **표준 MS
> 레시피 + 익명 파이프**로 자식이 정상 attach 한다(명명/overlapped 파이프 불필요).
> - **실측(office 박스, 2026-06-11)**: `_OwnedConPty` 실 클래스로 cmd.exe 띄워 배너 180B +
>   `echo` 마커가 **raw 바이트**로 우리 read 파이프에 도달, resize(`ResizePseudoConsole`)·
>   terminate·close·reap(종료코드 0) 전 수명 무예외. 손상 해결은 구성상 자명 — read 경로에서
>   디코드를 안 하고 raw 바이트를 서버 `pyte.ByteStream`(영속 incremental decoder)에 넘겨
>   바이트 경계를 carry → U+FFFD 미발생. write 도 raw `WriteFile`(③ 동시 해소).
> - **구현**: `conpty._ConPty`(ctypes `CreatePseudoConsole`/`ResizePseudoConsole`/
>   `ClosePseudoConsole`, 핸들 전부 `c_void_p` 로 64비트 절단 차단) + `pty_backend._OwnedConPty`
>   (리더 스레드 펌프, `_WinPty` 와 동일 모델). **opt-in**: `PYTMUX_PTY_BACKEND=owned|conpty`,
>   실패 시 조용히 pywinpty 폴백, 기본값은 검증된 pywinpty. 실 박스 라이브 검증 후 기본 전환 예정.
> - **헤드리스 도구 한계(→ 검증은 실 박스/실 제품)**: NonInteractive 도구 컨텍스트에선 어떤
>   백엔드도 자식 *출력*이 왕복 안 됨(pywinpty 도 conhost init 23B 만; `chcp 65001` 후 0 바이트
>   등 — 전부 probe 아티팩트, 백엔드 버그 아님). 그래서 멀티바이트 플러드 왕복은 **실 인터랙티브
>   박스의 라이브 제품**(`PYTMUX_PTY_BACKEND=owned`)에서 사용자 검증한다 — 절차는
>   [WINDOWS_TESTING.md](WINDOWS_TESTING.md). 헤드리스 단위는 선택 분기·env 블록·Windows 수명
>   (spawn→resize→terminate→close→reap)만 커버(`tests/test_pty_backend.py`).
> - **후속(필요 시)**: 익명 파이프는 attach·raw read 엔 충분하나 write 가 오버랩 불가라 대량
>   paste 버스트에서 블로킹 가능 → 그때 **오버랩 명명 파이프**로 승급(winpty-rs 가 명명 파이프를
>   쓰는 이유 = 취소·논블로킹 write, attach 때문이 아님). read 취소는 `ClosePseudoConsole` 의
>   EOF 로 충분.
