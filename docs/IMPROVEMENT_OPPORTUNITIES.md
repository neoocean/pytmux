...
### [M] 3.8 Claude Code 스크롤백을 '프롬프트' 단위로 묶어 히스토리로 표시 (요청 2026-06-12) 🚧 **Stage 1·2② 구현**
Claude 패널의 긴 raw 스크롤백을 사용자 **프롬프트** 경계로 탐색 가능하게 한다. 상세 설계·
anchor 수학·단계화는 [PROMPT_HISTORY_SCROLLBACK_SCENARIO.md](PROMPT_HISTORY_SCROLLBACK_SCENARIO.md).
...
- **Stage 1 완료**: 프롬프트 제출(두 경로: `_track_prompt`·스캔)마다 anchor 를 prompt_history
  ...
  통합 테스트 4건.
- **Stage 2 ② 완료**: 팝업에서 **Enter/행 클릭으로 직접 선택 점프**. InfoScreen 에 `select_cb`
  추가(없으면 기존 읽기전용) → `_open_prompt_history` 가 선택 행 인덱스를 `scroll_to_prompt`
  로 환산(`prompt-jump <n>` 과 동일, 서버 변경 0). 구분선/`[h]` footer 는 무시. 최신 프롬프트
  초기 선택이라 바로 Enter=최근 작업. 테스트 3건(`test_client`: Enter 점프·행 클릭 점프·footer
  무전송). `prompt-jump <n>` 타이핑 불필요.
- **Stage 2 잔여 후속**: ① 위로 스크롤 → 오버레이 자동 표시(raw 스크롤 대체 vs 별도 제스처 —
  제품 결정 대기), ③ 선택 '펼치기'(인접 anchor 로 구간 `[anchor_i, anchor_{i+1})` 분리 표시 —
  현재 ②의 점프가 그 프롬프트를 맨 위로 올려 대체로 충족). 시나리오 문서 §Stage 2.
