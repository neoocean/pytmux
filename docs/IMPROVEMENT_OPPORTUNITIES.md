...
### [M] 2.9 비활성 패널을 활성 패널 대비 옅게 표시 — 외곽선 없이 활성 패널 식별 (요청 2026-06-12)
한 탭에 패널이 둘 이상일 때 **비활성 패널의 모든 표시(글자·배경)를 활성 패널보다 한
톤 옅게(dim)** 렌더해, 외곽선(테두리 강조)에 의존하지 않고도 어느 패널이 활성인지
한눈에 알 수 있게 한다. **현재**: 활성 표시는 주로 테두리 색(divider/header)에 의존 —
색약·저대비 환경이나 테두리가 가려진 레이아웃에서 식별이 약하다. **개선(제안)**: 렌더
합성 시 비활성 패널 셀에 dim 변환(예: 전경/배경을 배경색 쪽으로 일정 비율 블렌딩하거나
ANSI dim 속성)을 일괄 적용. 패널 단위 1패스라 비용 작음. **주의**: ① 이미 dim 인 셀의
이중 적용·가독성 하한, ② Claude 등 색 의미가 중요한 앱의 색 왜곡 정도(요청은 "약간"),
③ copy-mode/검색 하이라이트는 활성 기준 유지, ④ ptyshot 골든은 색 속성까지 비교하므로
스냅샷 갱신 필요. 토글/세기 옵션(`inactive_dim = 0|1|세기`) 고려.
...
### [M] 3.8 Claude Code 스크롤백을 '프롬프트' 단위로 묶어 히스토리로 표시 (요청 2026-06-12)
Claude Code 패널을 위로 스크롤하면 이전 작업 **전체 raw 스크롤백**이 그대로 흘러가
원하는 지점을 찾기 어렵다. **목표**: 위로 스크롤 시 이전 기록을 **프롬프트(사용자
입력) 단위로 묶은 목록**으로 보여주고, 그중 하나를 선택해 펼치면 그 프롬프트로 진행된
기록(응답·툴 실행 등)만 보이게 한다 — "Claude Code 터미널을 위로 스크롤하면 프롬프트
히스토리가 보임". **현재 자산**: 서버가 이미 패널별 프롬프트를 추적한다(`claude_prompt`
파싱 → `p.last_prompt`/`p.prompt_history`, 헤더·`prompt-history` 팝업에서 사용). 이 목록을
스크롤백 경계 마커로 재활용할 여지가 있다. **개선(제안)**: ① 프롬프트 확정 시점의
스크롤백 오프셋(행 위치)을 함께 기록해 프롬프트↔구간 매핑을 만든다 → ② copy-mode/스크롤
진입 시 raw 대신 프롬프트 목록 오버레이(접힘)를 그리고, 선택 시 해당 구간으로 펼침/점프.
**주의/난점**: ① Claude 출력은 스트림이라 프롬프트 경계의 정확한 행 오프셋 포착이 비자명
(busy→idle 경계·`claude_prompt` 상승에지 활용), ② /clear·compact 로 스크롤백이 잘리면 매핑
무효화 처리, ③ 비-Claude 패널엔 비활성(이 뷰는 Claude 패널 한정), ④ pyte 스크롤백 한도와
상호작용. 별도 시나리오 문서로 설계 후 단계화 권장.
