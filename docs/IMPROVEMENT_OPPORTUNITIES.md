...
- **예산/임계 알림**: ~~usagelog 일/주/월 집계는 있으나 예산 초과 경고가 없어 수동 조회~~
  ...
  **해결(검증 2026-06-14, stale 정정)**: 시나리오 §4 T3 가 **M13·M14c 로 구현 완료**다.
  ① **plan 유도(M13)**: `claude_budget_plan` opt-in + 실측 게이트 ≥80% + idle 에서
  `_drive_perm_mode(p,txt,"plan")`(bypass 불간섭·이미 plan 무동작·auto 보다 우선,
  `servermixin.py:1163`). ② **모델 힌트(M14c)**: `claude_model`/`model_overselect_hint`
  (Opus 계열 + 반복≥3 + 잔량≥40 여유 → "가벼운 모델 고려" 배지, **알림만·자동 전환 없음**),
  opt `claude_model_hint`(기본 OFF·`model-hint` 명령). "Haiku 강제"는 S4 설계상 비채택
  (자동 모델 전환=작업 의미 변경 위험 → 힌트만). 테스트 `test_token_saver.
  test_budget_plan_induction`·`test_model_overselect_hint`.
