...
>   부재) **+ 612 green**.
>
> **배지 위치**: ` REC ` 배지는 **시스템 배지(SYNC/AR) 직후**(종전과 동일 위치)에 렌더된다.
> 코어 `_render_main` 이 시스템 배지 영역에서 **`client_statusbar_badges`** 훅을 호출하고
> rec 가 그 훅으로 REC 배지를 그린다(좌하단 정보 클러스터 `client_statusbar` 보다 앞).
> 두 훅 모두 P6 폭-체이닝 규약(w0 입력·새 폭 반환)을 따른다.
>
> 아래 §4 표·§8 단계는 **작성 시점(미구현) 스냅샷**이라 본문은 보존한다. 실제 코어
> 접점 라인은 위 추출로 드리프트했고, 다수 항목은 기존 훅(server_init/server_opts_*/
> server_status/server_command)에 매핑돼 새 훅은 `server_pty_output`+`server_shutdown`
> 2종만 추가됐다(§4.3 권고와 일치).
