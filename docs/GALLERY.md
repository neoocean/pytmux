...
> 전제는 키 인증 또는 ControlMaster 설정이며, 원격 실행은 바깥 remote-attach 로 자동 승격됩니다.
...
토큰 사용량(`token-log`) — 노트북 탭(기간/계정/세션/한도/대사/경고)+서브옵션+요약줄.
한 번에 한 차원만 보이는 2026-06-12 재설계:

![토큰 사용량 팝업 — 일별 뷰](image/24-token-log.svg)

시간 뷰 — 시각별 세션 5h 한도 누적%를 초록/노랑/빨강 **계단식 가로 막대**로, 옆에 주간
(1w%) 열을 둡니다(권위 `/usage` 실측을 시각 단위로 조인):

![토큰 팝업 — 시간별 5h 한도 계단식 막대](image/37-token-log-hour.svg)

한도 뷰 — `/usage` 막대(세션 5h·주 전체·주 Sonnet)·현재 창 추정 Σ·다음 리셋 블록-숫자
카운트다운을 한 탭으로 통합:

![토큰 팝업 — 한도(/usage) + 리셋 카운트다운](image/38-token-log-limit.svg)
