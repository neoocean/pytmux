...
> **구현 노트(실제 채택 — 무위험 변형)**: 코드 확인 결과 ① 토글 핸들러 중 12개
> (예산/컨텍스트)와 `set_claude_rules` 는 이미 `_broadcast_session`(→ `_send_full`,
> full=True)으로 변경을 회신하고, ② 신규 attach 도 `_send_full` 로 full status 를
> 받으며, ③ 클라 `update_status` 가 이미 `msg.get(k, self.k)` 로 **키 부재 시 직전 값을
> 유지**한다(B11 류). 따라서 이 13개를 **full status 에만** 싣고 주기에서 빼면 동작이
> 증명적으로 불변(클라 코드 변경 0). 반면 `claude_header`/`single_border`/
> `auto_doc_clear`/`claude_auto_mode` 4불린은 전용 브로드캐스트 없이 낙관적 토글+주기
> 화해에 기대므로 **주기에 그대로 유지**(behavior-neutral 우선). 가장 무거운
> `claude_rules`(문자열)와 예산/컨텍스트 12필드의 30Hz 재전송이 제거된다.
...
| C4 status 정적 옵션 분리 | ✅ 구현 | `serverio._status_msg` 가 토글 전용 정적 옵션 13개(`claude_rules`+예산/컨텍스트 12)를 **full status 에만** 싣고 주기(full=False)에선 생략. 이들은 모두 `set_*`→`_broadcast_session`(full=True)으로 회신하고 attach 시 full 을 받으므로 도달 보장, 클라는 retain-on-omit(`update_status` 의 `msg.get(k,self.k)`)으로 유지 — 클라 변경 0. 낙관 토글 4불린(header/single_border/auto_doc_clear/auto_mode)은 전용 브로드캐스트가 없어 주기에 유지. 회귀 `test_status_static_opts_only_on_full_c4`(서버)·`test_status_retains_static_opts_when_omitted_c4`(클라). |
