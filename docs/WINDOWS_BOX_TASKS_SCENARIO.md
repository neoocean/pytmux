...
- **재개 시도 & 기각(2026-06-10)**: 위 (b) 를 구현했다 — write 전에 각 패널이 출력을
  낼(=stdin 준비) 때까지 best-effort 폴링 대기를 넣고 Windows skip 을 해제. 그러나 전체
  `tests/test_server.py` 누적 실행에서 **여전히 3회 재시도 모두 FAIL**(SYNCED 미수신).
  셸-준비 대기로도 안 닫힌다 → 원인이 cmd 프롬프트 타이밍보다 깊은(장수 프로세스의
  conhost 핸들/이벤트루프 누적 상태) 것으로 재확인. **skip 유지가 결론.** 다음 시도는 (b)
  가 아니라 (a) bisect(어느 선행 테스트가 오염시키는지) 또는 매 테스트 server 프로세스를
  완전 격리(서브프로세스 분리)하는 하네스 변경 쪽이어야 한다.
