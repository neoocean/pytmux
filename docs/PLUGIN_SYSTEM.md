...
현재 저장소에 플러그인 **8개**가 있다(작성법·축별 사례·스크린샷은
[PLUGIN_MANUAL.md](PLUGIN_MANUAL.md), 화면은 [GALLERY.md](GALLERY.md)):
...
| `p4-show-submitted-changelists` | 퍼포스 submitted CL 목록 풀스크린 | 모달 + 서버 왕복 |
| `claude-code` | Claude Code 통합(스캔·상태·토큰·렌더 전체) | 모든 축(아래 §4) |
...
- **`ncd`**(완료, CL 57709/57721/**57774**, cwd 표시 **57803**): Norton Change Directory
  ...
  회귀: `tests/test_nc.py`.
- **`claude-code`**(완료 — Phase 1~3 추출 종료, 아래 §4): 코어 완전 delete-to-disable.
...
- **Phase 2c 완료**: 클라이언트의 모든 Claude 렌더/상태/클릭존을 플러그인으로 이전,
  ...
  - **검증 한계 보강**: driver 로는 client.py 가 안 도므로 `tests/test_client.py`(+contract)
    스위트가 회귀망(현재 전체 591 passed). 이전된 심볼의 테스트 import 경로도 갱신(예: `_toggle_remote_
    control` 의 write_msg 패치를 `pytmuxlib.protocol` 로).

### Phase 3 S4/S5 물리 이전 — 완료(2026-06)
- **완료**: `claude.py`·`tokens.py`·`usagedb.py`·`usagelog.py`·`usageprobe.py` 는 전부
  `pytmuxlib/plugins/claude-code/` 로 물리 이전됐고(코어 루트엔 없음), 코어 모듈은 이들을
  더는 import 하지 않는다(`grep` 확인 — 0건). Pane 의 Claude 거동 필드도 플러그인
  `pane_init`/`panestate` 가 설치한다(S4 — `model.Pane.__init__` 주석 참조). 즉 코어는
  Claude 를 전혀 모르며, `plugins/claude-code/` 디렉토리를 지우면 서버·클라 모두 에러 없이
  일반 멀티플렉서로 남는다(완전 delete-to-disable).
