...
- **남은 Phase 2c (클라 렌더)**: `_draw_claude_headers`(프롬프트 헤더), 상태줄 토큰/사용량/
  모델/예산 그리기 + 클릭존, `_scan_footer_zones`, ESC `_handle_hdr_focus`/`_status_buttons`
  의 Claude 항목, 통합 상태탭(REC/토큰/서버 혼합)의 토큰 탭.
  → **클라 렌더/ESC-nav 레지스트리 훅**(`composite_overlay`/`on_click`/`esc_nav` 등)
...
- (남은) Phase 2c(헤더/배지 렌더·ESC-nav) · (선택) Phase 3 S4/S5 물리 이전 — §4·[HANDOFF.md](HANDOFF.md) §11.6
