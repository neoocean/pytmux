...
> **상태**: 시나리오(미착수, 2026-06-10 작성). **선행 조건 충족** — S5 토큰 모듈화
...
1. **실측 % 게이트 기본값**: `usage_gate_session_pct`/`usage_gate_week_pct` 의
   기본을 끔(0)으로 둘지, 보수적 기본(예: 세션 95)을 줄지 — T2 대사 데이터와
   실사용 후 결정. (절감 자동화 M13 무장 임계 80 과의 정합도 함께.)
2. **limits 스냅샷 보존 기간**: prune 기준(예: 90일) — usage 테이블 보존 정책과
   별도로 둘지.
3. **추정 분모 경로 폐기 여부**: 실측 % 가 1차가 되면 `_learned_5h_cap`·
   `token_budget_5h` 분모 근사(`_tok5h_pct` ②)는 죽은 코드에 가깝다 — T3 에서
   같이 걷어낼지, 실측 공백 폴백으로 남길지.
4. **기존 `token_budget_*` 절대 예산의 장기 운명**: T4 에서 보조 축 유지 후,
   실측 게이트가 검증되면 deprecate 할지(설정 마이그레이션 shim 필요 — S5 T3
   선례).
5. **상태줄 추정 누계 노출 수위**: `tok ~Σ` 로 남길지, 기본 숨김(팝업에서만)으로
   갈지 — 사용자 결정.

---

> **다음 행동**: T1(스냅샷 영속화)부터 착수 — 표시·게이트 무변경의 순수 추가라
> 위험이 가장 낮고, T2 와 함께 이후 모든 결정(§7-1·3·4·5)의 데이터 기반을 만든다.
> 착수 전 §1.3 표를 grep 으로 재검증할 것.
