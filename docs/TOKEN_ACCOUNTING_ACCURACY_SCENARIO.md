...
- **상관**: 원본 피어슨 r=0.049(중복 이상치 지배) → 중복 접기(dedup) 후
  **r=0.376**(n=18, 비-reset). 여전히 약한 이유는 구조적: 실측 5h% 는 **계정
  ...
  에만 유효).
- ~~**잔존 관찰 항목 ①** limits/usage 계정 식별 전부 None/'unknown' 원인~~ →
  **해소(같은 날 후속 CL)**. 원인: 계정 라벨(`Organization:`/`Email:`)은 설정
  패널의 **Status 탭(/status)에만** 있고 부팅 화면·Usage 탭(/usage)엔 아예 없다
  (숨은 세션 실험 + 실캡처 fixture 로 확정 — 파서 문제가 아니라 원천 부재).
  수정: ⓐ 프로브가 계정 미식별 시에만 Esc+`/status` 1회 추가 스크랩(같은 숨은
  세션 재사용·토큰 0) → limits 스냅샷 계정 채워져 게이트 계정 대조·같은 계정
  한정 대사 활성. ⓑ 패널 화면엔 여전히 라벨이 안 떠 usage 레코드는 'unknown'
  이 정상 — reconcile 같은-계정 합산에 미식별 레코드를 **포함**(§10-B 단일 계정
  귀속과 같은 가정; 빼면 같은 계정 활동이 Σ=0 으로 왜곡).
- **잔존 관찰 항목**: ② 중복 가드 적용 **후** 데이터로 상관 재관찰(이날 데이터는
  dedup 근사로만 평가 — 계정 식별 활성화 이후 같은 계정 한정 대사로).
- ~~③ 누적 DB 이력의 중복 정리~~ → **해소(같은 날 후속 CL)**: usagedb **v3
  데이터 마이그레이션** — connect 시 1회, 잔상 런(같은 pane·session·tokens 60초내
  연쇄)의 첫 건만 남기고 `usage_dup_archive` 로 격리(하드 삭제 아님 — 포렌식
  보존, 집계는 usage 만 봄). 실패 시 user_version 유지로 다음 connect 재시도.
  라이브 반영은 서버 재시작 후 첫 DB 접근 시점.
