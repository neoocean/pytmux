...
> 전체 463 green. 남은 후속: §7-4(절대 예산 deprecate 여부 — 실사용 검증 후)와
...
4. **기존 `token_budget_*` 절대 예산의 장기 운명**(잔존 열린 질문): T4 에서 보조
   축으로 유지 — 실측 게이트가 실사용에서 검증되면 deprecate 검토(설정
   마이그레이션 shim 필요 — S5 T3 선례).
...
> Δ% vs 추정 Σ 상관을 관찰 — 추정 강등의 실증 데이터 축적(§5-1·2). ② 실사용
> 검증 후 §7-4(절대 예산 deprecate) 결정. ③ 서버 재시작(restart-server)으로
