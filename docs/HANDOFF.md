...
- **좌하단 Claude 토큰 사용량을 같은 계정의 모든 탭·패널 합계로 고정** (2026-06-11
  요청). 요구사항: **같은 계정이면 탭/패널을 전환해도 좌하단 합계 숫자가 변하지
  않고 동일**해야 한다. 현행 서버 `_account_token_total(ap)` 은 활성 패널의
  계정 기준 합산이지만, 활성 패널의 계정이 미식별(None)이면 그 패널 단독 누계로
  떨어지고, 비-Claude 패널에선 `claude_active` 게이트로 숨는 등 전환 시 값이
  출렁인다. 착수 시 확인: ① 계정 미식별 패널에서의 폴백 규칙(마지막 식별 계정
  유지?), ② 탭 전환·비-Claude 패널 활성 시 표시 유지 정책, ③ 멀티 계정 동시
  사용 시 어느 계정 합계를 보일지(활성 패널 계정 우선), ④ 테스트(전환 전후 동일
  값 단언).
