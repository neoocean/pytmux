...
- **[리팩토링 요청, 미구현] 코드를 LLM 친화적인 형태로 리팩토링** — 요청: 코드를
  ...
  **공유 워크스페이스 충돌 주의**(§9/§11 의 attribution 경고 — Windows 포팅 등 동시
  ...
  사용자 확인 또는 별도 설계 필요. **연관**: §11 전체. **현재는 기록만 — 미구현.**
...
- **[UI 요청, 미구현] 토큰 사용량 표시 — 기호와 숫자 사이 한 칸 띄우기** — 요청:
  ...
  반영. **현재는 기록만 — 미구현.**
- **[UI 요청, 미구현] `help` 명령에도 자동완성 후보 박스 표시** — 요청: 명령 프롬프트
  ...
  **부분일치**해 후보 박스(`#pcand`)를 채운다 — 그런데 **`help` 은 `COMMANDS` 목록에
  ...
  관계 정리 필요(help=인라인 후보 박스 vs ?=전체화면 목록). **현재는 기록만 — 미구현.**
- **[UI 버그, 미구현] 명령 프롬프트 표시 시 배경 디밍이 일부 글자에 충분히 적용 안 됨** —
  ...
  vs 자체 cells 합성). **현재는 기록만 — 미구현.**
- **[UI 요청, 미구현] 배경색이 팝업 색과 같으면 배경을 조금 더 어둡게 해 팝업과 구분** —
  ...
  디밍 항목·팝업 높이 항목들. **현재는 기록만 — 미구현.**
- **[UI 요청, 미구현] `esc :` 로 여는 명령어 프롬프트를 외곽선(테두리)으로 둘러싸기** —
  ...
  `PromptScreen`)에도 테두리를 줄지 결정 — command 만 vs 전체. **현재는 기록만 —
  미구현.**
- **[UI 요청, 미구현] 명령 목록 등 팝업 — 세로 길이를 "최대 내용" 상태에 고정하고
  ...
  **아래쪽을 빈 채로** 둬 높이를 고정하라. 현재 구현: `CommandListScreen` CSS
  ...
  InfoScreen 팝업 높이 항목. **현재는 기록만 — 미구현.**
- **[UI 요청, 미구현] 프롬프트 히스토리 등 InfoScreen 팝업 — 배경 탭 패널 안인 한
...
- **[기능 요청, 미구현] 하단 토큰 사용량 표시 — 지속 표시 + 계정별 전체 세션 합계** —
  ...
  **현재는 기록만 — 미구현.**
- **[기능 요청, 미구현] Claude Code 패널 감지 시 권한모드를 자동으로 "오토모드"로
  전환** — 요청: 패널에서 Claude Code 가 떠 입력 대기(idle) 상태가 되면, 현재
  ...
  클라 `send_input`/서버 PTY write). **현재는 기록만 — 미구현.**
- **[UI 요청, 미구현] 패널 출력 캡처(REC) 정보 팝업 — 팝업 바깥을 클릭하면 닫기** —
  ...
  팝업 모두**에 적용된다 — 의도와 맞는지 확인(보통 바람직). **현재는 기록만 — 미구현.**
- **[UI 요청, 미구현] 패널 중앙 달력 오버레이에 외곽선(테두리) 그리기** — 요청:
  ...
  유지. **현재는 기록만 — 미구현.**
- **[UI 요청, 미구현] 패널 우상단 닫기 [x] 버튼을 패널 외곽선 *안쪽*으로 이동** —
