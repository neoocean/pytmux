...
- **상태**: `docs/FEATURES.md` 의 모든 항목 구현. 헤드리스 테스트 **586 passed**
...
- **SQLite strftime `%G`/`%V`(ISO 연-주)는 3.46+(2024) 전용**: `usagelog` 의 week 버킷
  키는 파이썬 `strftime('%G-W%V')` 인데, SQLite 에서 같은 키를 `strftime` 으로 만들면
  구버전(번들 sqlite3)에선 빈 문자열/오류라 **클라-서버 집계가 안 맞는다**. 그래서
  토큰 Phase B 서버 GROUP BY(`usagedb`)는 **버킷별 집계를 SQL 로 옮기지 않고** 전체
  이력 합(`total_all`/`totals_by_account`, `%`/`%V` 불필요)만 SQL 로 하고, 버킷×차원
  전환은 클라측(`usagelog.aggregate`) 유지(정확·즉시·라운드트립 없음). hour/day/month
  는 `%Y%m%d%H` 로 안전하나 week 한 종류 때문에 일관성 위해 버킷 전체를 클라에 둔다.
...
- **token-usage→token-log 통합 + 사용량 막대 '사용' 라벨(2026-06-12, CL 58556/58559)** — 06-12
  ...
  - **58556**: `token-usage` 트리 팝업·통합 상태 팝업의 '토큰 사용량' 탭 제거 — token-log
    팝업으로 일원화(세션별 ~Σ 는 token-log 세션 뷰가, 라이브 ctx% 는 상태줄·헤더가 커버).
    명령은 별칭으로 잔존(`token-usage`/`tokens` → token-log). 통합 상태 팝업은 REC·서버
    2탭(host 클릭 initial=2 는 기존 클램프로 서버 탭 안착). tree purpose=="usage" 죽은
    분기 제거.
  - **58559**: usage_bar_lines % 에 '사용/used' 라벨 — 팝업 막대(사용률 10%)와 footer
    잔여 표기("90%/5h 남음")가 다른 값처럼 읽히던 혼동 해소(같은 실측의 보수 관계 —
    값 불일치 아님, 사용자 보고로 확인). Claude /usage 원문("N% used")과 동일 표기.
    right_align(usage-view 오버레이)은 폭 정렬 계약이 있어 무변경.
...
- **git 미러 캐치업 → p4 @58401(2026-06-12)** — p4 sync 후 git HEAD(`bd22163`=CL 58374)가
  ...
  발견성 FEATURES.md 문서 정정(코드·i18n 부분은 앞선 미러에 이미 반영, 문서만 누락분 따라잡음)
  · **58376** 본 §9 IMPROVEMENT 배치 핸드오프 기록 · **58385** §2.8 탭 인덱스 음수·범위밖 알림
  · **58401** 토큰 사용량 팝업 일/주/월 버킷 전체 이력 집계. push 시 원격이 CI 벤치 자동동기화
  2건(`docs/benchmark/`만, `[skip ci]`) 선행 → fetch 후 리베이스(충돌 없음)·push. **교훈**(56321
  과 동일): git HEAD 번호가 depot 최신이어도 미러가 **개별 파일·문서 부분을 선별 누락**할 수
  있으니(여기선 58350 FEATURES.md), 캐치업은 git HEAD 의 CL 번호가 아니라 워킹트리 실제 diff
  기준으로 매핑할 것. `p4 opened` 비어 로컬 미서브밋 작업 없음 확인 후 진행.
...
> ⚠️ **이 목록은 stale 일 수 있다 — "남은 작업"으로 보고하기 전 반드시 코드/테스트로
> 검증하라(2026-06-09 교훈).** 이 섹션의 "원래 보고·분석"·"구현 방향"·"미구현" 항목 다수가
> 실제로는 **이미 구현·테스트 완료**였는데 분석 메모만 안 지워져 남아 있었다. 한 세션에서
> §10 을 그대로 "남은 작업"으로 제시했다가, 막상 착수하니 #1·#2·#4·#5·#6·#8·#9·#10 이
> 전부 이미 있었다(코드+회귀 테스트 존재, p4 57667~57674 로 항목을 해결 표시). **원인**:
> 기능이 ship 될 때 구현 CL 은 남기지만 §10 의 해당 분석 항목을 `~~취소선~~`+"→ 해결"로
...
- ~~**좌하단 Claude 토큰 사용량을 같은 계정의 모든 탭·패널 합계로 고정**~~
  ...
  계정 미식별(None) 패널에선 활성 패널 단독 누계로 폴백해, 식별 패널(계정 합계)과
  오갈 때 좌하단 숫자가 출렁였다. 채택 규칙: ① 식별 계정이 **하나뿐**(또는 전부
  미식별)이면 단일 계정으로 보고 미식별 패널까지 포함해 Claude 흔적 있는 전
  패널 합산 — 어느 탭/패널이 활성이어도 같은 숫자. ② 계정이 둘 이상 식별된
  상태에선 활성 패널 계정의 합계(미식별 패널은 임의 귀속하지 않음), 미식별 패널
  활성이면 0 전송 — 클라 지속표시가 마지막 값을 유지해 표시는 그대로. 비-Claude
  패널은 기존대로 `claude_active` 게이트로 숨김(정책 유지).
  `test_account_token_total_aggregates_across_panes` 에 전환 전후 동일 값·탭
  전환·다계정 미귀속 단언 확장.
...
- ~~**[UI 요청] 패널 내 `Remote Control active` 클릭/터치 → 원격제어 토글 팝업**~~ →
  **해결(토글 구현 완료)**. 클릭→`InfoScreen` 팝업(CL 56602) + **[r] 키 토글 추가**.
  이전 결론("터미널서 직접 토글 수단 없음")은 **틀렸다**(사용자 보고) — Claude Code CLI 의
  **`/rc` 슬래시 명령**으로 원격제어를 켜고 끈다. 팝업의 `[r]` 가 해당 패널에 `/rc`+Enter 를
  주입(`_toggle_remote_control` → input 메시지 → 그 패널 PTY). 안내문도 정정. 아래는 원 분석.
...
- ~~**[리팩토링 요청] 코드를 LLM 친화적인 형태로 리팩토링**~~ → **핵심 목표 달성**(거대
  ...
  serve 등). ⑦ **세션/window/tab/패널 트리 조작 → `servertree.py` `ServerTreeMixin`**
  (CL 56779 — split/kill/move/swap/rotate/break/join/zoom/rename·popup·레이아웃 프리셋·
  동기화). **최종: server.py 2968→567줄(-81%), 믹스인 7개**(serverclaude 585/serverio
  693/servertree 602/serverpersist 313/serverpty 215/servercapture 106). 남은 server.py
  본체(567줄) = __init__·세션 부트스트랩·버퍼/붙여넣기·토큰 로깅·옵션 토글·탭 레이아웃
  슬롯·handle_control·pipe/capture/search/resize·세션 헬퍼 — 작고 잡다해 추가 분리
  이득이 작아 Server 에 유지. **LLM 친화 리팩토링 핵심 목표 달성**(거대 단일 파일 해소·
  Claude 코드 모듈 분리). 분리 패턴: 연속
