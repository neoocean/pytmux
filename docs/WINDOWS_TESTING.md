...
| **GitHub Actions `windows-latest`** | ✅ (클라우드 실행) | ✅ pywinpty·ConPTY 실제 | ★★★ **권장** |
...
**GitHub Actions의 `windows-latest` 러너**가 이 프로젝트에 가장 잘 맞는다 — 실제 Windows
커널 위에서 `pywinpty`를 설치하고 헤드리스 스위트 + `scripts/win_report.py`를 그대로
돌려 리포트를 아티팩트로 남길 수 있다.
...
## 3. 권장 시나리오 — GitHub Actions `windows-latest` CI

이 저장소는 **Perforce 주(主) + GitHub 미러(`neoocean/pytmux`)** 다. Actions는 **미러에
푸시된 커밋**에 대해 돈다(미러가 depot보다 늦을 수 있음 — 그래도 주기적 Windows 검증엔 충분).

### 3-a. 워크플로 (`.github/workflows/windows.yml`)
...
jobs:
  windows:
    runs-on: windows-latest    # Windows Server 2022, x86-64
    strategy:
      matrix:
    ...
    steps:
      ...
      - name: Install deps (incl. pywinpty)
        run: |
          python -m pip install --upgrade pip
          python -m pip install -r requirements.txt

      - name: Smoke — pywinpty imports + ConPTY spawns
        run: |
          python -c "from winpty import PtyProcess; p=PtyProcess.spawn('cmd /c echo ok'); print('pywinpty OK')"

      - name: Headless test suite
        run: python tests/run.py

      - name: Windows compat + perf report
      ...
      - name: Upload report artifact
        ...
        with:
          name: win-report-${{ matrix.python }}
          path: reports/win-report-*.md
```

### 3-b. 이 워크플로가 잡는 것
- 3개 Python(3.11/3.12/3.13)에서 **pywinpty 설치·import·ConPTY spawn** 회귀.
- 헤드리스 스위트 전체(현재 202 passed)가 **Windows에서도** 통과하는지.
- `win_report.py`가 만든 **호환성·성능 Markdown 리포트**를 아티팩트로 남겨 macOS에서
  다운로드해 본다(이전 실행 대비 델타는 `win-report-latest.json` 기반).
