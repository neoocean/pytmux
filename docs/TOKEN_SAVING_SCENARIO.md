...
| **M16** | **PTY 밖 에스컬레이션 훅**(T6) — `claude-budget-warn/over`·`claude-auto-armed`·`claude-limit` 훅(클라 status 전이에 발화) | 0 | 낮음 | `client.py`(status 핸들러 529·`_fire_hook` 480·`__init__`); 서버 무변경 | 📐 **설계확정(§8)** |
...
| **M18** | **상태줄 가시성 + 통합 팝업**(§9) — `ctx N%/1M`·`25k(7%/5h)` 표기 + 사용량존 클릭/esc-Enter→통계+시나리오 토글 팝업 | 0 | 낮음 | `claude.py`·`usagelog.py`·`clientwidgets.py`·`clientscreens.py`·`client.py` | 📐 설계(§9) |
...
## 8. 다음 구현 기능 — 상세 설계 [확정] M16 / T6: PTY 밖 에스컬레이션 훅
...
**전이 디바운스(§5.6):** `__init__` 에 `self._prev_budget_level = 0`,
`self._prev_pending_kind = None`, `self._prev_limit = False` 를 둔다. status 마다 현재값과
비교해 **상승 에지에서만 1회 발화**(하강·동일은 무시). `alert-bell` 의 `_prev_bell` 패턴과
동일 — 같은 화면을 여러 프레임 봐도 중복 발화하지 않는다.

**컨텍스트 전달:** `_fire_hook` 가 지금은 셸 문자열만 실행한다(`_run_command(cmd)`). 한 줄
확장으로 **환경변수**를 실어 사용자 명령이 상황을 알게 한다:
...
미바인딩 = 무동작(옵트인). 예시:
```
set-hook claude-budget-over 'terminal-notifier -title pytmux -message "예산 초과 $PYTMUX_ACCOUNT"'
set-hook claude-auto-armed  'osascript -e "display notification \"$PYTMUX_PENDING_KIND ${PYTMUX_PENDING_ETA}s\" with title \"pytmux\""'
set-hook claude-limit       'afplay /System/Library/Sounds/Glass.aiff'
```
...
## 9. 가시성 기능 — 상태줄 사용현황 표시 + 통계/설정 팝업 [설계]
...
**설계**:
1. **진입(마우스)**: 사용량존은 이미 클릭존이다(`_usage_zone` 바운드, on_mouse_down
   clientwidgets.py:1098-1104 → 현재 TokenLogScreen). 이 핸들러를 **통합 팝업**으로 돌린다.
2. **진입(esc/scroll 모드)**: 현재 scroll 모드의 Enter 는 그냥 모드를 빠져나간다
   (client.py:2885). 보강: Enter 시 **커서가 상태줄(하단행)의 `_usage_zone` x 범위 안**이면
   모드 종료 대신 통합 팝업을 연다(기존 마우스 히트테스트 x 바운드 재사용 — 상태줄
   클릭은 하단행 한정, clientwidgets.py:1077). 그 외 위치 Enter 는 기존대로 종료.
3. **통합 팝업**: 위(통계) + 아래(시나리오 토글) 한 화면.
   - 통계부: TokenLogScreen(clientscreens.py:909) 내용 — 버킷×계정 집계
     (`usagelog.aggregate`) + **9.2/9.3 의 ctx%·5h 근접도 요약 라인**.
   - 설정부: ClaudeSaverScreen 의 토글 행(SAVER_ROWS, clientutil.py:298-311) 재사용 —
     `●/○` 토글·프리셋 순환, `_saver_action`(client.py:1743) 으로 서버에 그대로 송신.
   - 두 화면을 합치거나(권장: 통계 위 + 토글 아래 한 스크린), 최소안으로 통계 팝업에
     **`[s] 시나리오 설정` 키**를 둬 ClaudeSaverScreen 으로 점프. 어느 쪽이든 **"사용량을
     보고 → 그 자리에서 절감 on/off"** 동선을 만족.
...
| 위치 | 변경 | 신규/재사용 |
|---|---|---|
| `claude.py` `claude_usage`(77) | %+배지 동시 시 `ctx N% / 1M` 합성(A) | 소폭 |
| `usagelog.py`(80) | `ts≥now−5h` 롤링 5h 누계 집계 변형(B 분자) | 신규 소 |
| `serverclaude.py`/opts | `token_budget_5h` opt + limit 관측 학습(B 분모) | 신규 |
| `serverio.py`(133) | status 에 `ctx_pct`·`tok_5h_pct`(또는 합성문자열) 추가 | 소폭 |
| `clientwidgets.py`(990·1098·1077) | 표기 합성(A·B) + 진입 핸들러를 통합 팝업으로(C) | 재사용+소 |
| `clientscreens.py`(909·368) | 통계+토글 통합 팝업(또는 `[s]` 점프) | 재사용 |
| `client.py`(2885) | scroll 모드 Enter 가 usage_zone 위면 팝업(C) | 소폭 |

### 9.7 검증

- **단위**: `claude_usage` 합성(%+배지/한쪽만/없음), 롤링 5h 누계(경계 ts 포함/제외),
  분모 3경로(설정/학습/미상→무표기), esc-Enter 히트테스트(usage_zone 안/밖).
- **골든 픽스처(§7)**: ctx% 표기 의미(headroom vs used) 확정 — A 의 `100−` 변환은 이게
  고정돼야 안전.
- **실 박스 1회(사람)**: 팝업 진입(클릭·esc-Enter)·토글 반영 육안 확인.

> **요약**: 9.2·9.3 은 표시 합성(데이터는 이미 있음, 단 5h 분모만 정직히 best-effort),
> 9.4 는 흩어진 통계·설정을 토큰 영역 한 진입점으로 모으는 동선 개선. 전부 등급 0 이라
> §8(M16) 다음 또는 병행으로 안전하게 넣을 수 있다.
