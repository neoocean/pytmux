...
**실제 화면을 텍스트로 재현한 "스크린샷"** 과 함께 차근차근 설명합니다.
...
![pytmux 첫 실행 — 단일 패널·상단 탭바·하단 상태줄](img/01-first-run.svg)
...
![좌우 분할 — 활성 패널은 파란 테두리, 비활성은 회색](img/02-split-lr.svg)
...
![중첩 분할 — 세 패널, ┬┴├┤ 로 이어진 경계](img/03-split-nested.svg)
...
![줌 — 한 패널이 전체를 채우고 상태줄에 Z 표시](img/04-zoom.svg)
...
![패널 닫기 — 하단에 kill-pane? (y/N) 확인 프롬프트](img/07-kill-pane-prompt.svg)
...
![탭 여러 개 — 활성 탭 강조, 이름변경한 build 탭, 오른쪽 [+]](img/08-tabs-multi.svg)
...
![탭 닫기 확인 — 중앙 확인 박스](img/10-confirm-tab.svg)
...
키보드로 스크롤백 모드에 들어가시려면 `prefix [` 를 누릅니다.
...
![메뉴 — 분할·줌·삭제·동기화·새 탭·레이아웃·detach 등 전체 항목](img/05-menu.svg)
...
![명령 프롬프트 — 하단 입력창에 split-window 입력, 회색 고스트 자동완성](img/06-command-prompt.svg)
...
비활성 탭의 작업이 끝나면 **탭 배경색으로 완료를 알립니다**(그 탭을 보시면 해제됩니다).
...
- `prefix R` — 패널마다 켜고 끕니다. 켜진 패널은 상태줄에 `AR` 표시가 뜹니다.
...
![달력 오버레이 — 이번 달 달력, 오늘 날짜 강조](img/09-calendar.svg)
