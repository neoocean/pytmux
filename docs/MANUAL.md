...
탭을 닫을 때는 **중앙 확인 박스**가 뜹니다(마지막 탭이면 "pytmux 가 종료됩니다" 경고).

![탭 닫기 확인 — 중앙 확인 박스](image/10-confirm-tab.svg)
...
Claude 패널 맨 윗줄에 **지금 처리 중인 프롬프트**를 다른 색 스티키 헤더로 보여줍니다
(스크롤과 무관하게 고정). 큐에 대기 중인 프롬프트는 처리 시작 시 반영됩니다.

![Claude 처리중 — 탭바 ◐ 아이콘, 파란 스티키 헤더(처리 중 프롬프트), 하단 상태줄 토큰](image/11-claude.svg)

위 화면에서 탭바의 `◐ 1:claude`(처리중), 맨 위 파란 **스티키 헤더**("리팩터링하고
테스트 추가해줘"), 하단 상태줄의 토큰(`12.3k tok · Σ 1,900`)을 한눈에 볼 수 있습니다.
...
활성 Claude 패널의 토큰/컨텍스트를 상태줄에 표시하고(위 [§11.2](#112-스티키-프롬프트-헤더)
스크린샷 하단 왼쪽의 `12.3k tok · Σ 1,900`), **클릭하면 실행 중인 Claude 탭/패널 +
사용량 트리 팝업**이 열립니다.

사용량은 세션 동안 유지·누적되며 `token-log` 로 파일에 영속 기록하실 수 있습니다.
...
- `prefix t` 또는 하단 바 **시계 클릭** → 시계 모드(현재 패널을 큰 시계로 덮습니다).
- 하단 바 **날짜 클릭** 또는 명령 `calendar-mode`/`cal` → 이번 달 달력 오버레이.

![달력 오버레이 — 이번 달 달력, 오늘 날짜 강조](image/09-calendar.svg)
...
새 이미지로 교체**합니다(제자리 re-exec). 서버를 업데이트하면서도 작업을 잃지 않습니다.
