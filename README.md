...
> **[사용 매뉴얼 → 위키](https://github.com/neoocean/pytmux/wiki/User-Manual)** 를 보세요. 이 README 는 프로젝트
> 소개입니다.
>
> - 🌐 **[소개 사이트 → pytmux.woojinkim.org](https://pytmux.woojinkim.org/)** — 핵심 가치·기능·Claude
>   연동·화면을 한 페이지로 둘러보기(설치~플러그인 **[상세 가이드](https://pytmux.woojinkim.org/guide.html)** 포함).
> - 🖼️ **[화면 갤러리 → 위키](https://github.com/neoocean/pytmux/wiki/Screenshots)** — pytmux 와 플러그인들의
>   주요 화면을 한눈에(스크린샷 + 짧은 설명).
> - 🔌 **[플러그인 매뉴얼 → 위키](https://github.com/neoocean/pytmux/wiki/Plugin-Authoring-Guide)** — 기본 제공
...
- tmux 와 비슷하게 동작하지만, **명령어를 다 외우지 못하는 사람**과 **마우스를 좀 더
  ...
  - 🧭 **TUI 기반 메뉴** — 단축키를 외우지 않아도 메뉴(`prefix Enter` 또는 우클릭)와
    명령 프롬프트(`prefix :`)로 거의 모든 동작을 할 수 있습니다. 메뉴는 패널·레이아웃·탭
    **그룹(서브메뉴)** 으로 정리되어 있습니다.
...
- **플러그인 시스템(delete-to-disable)**: 시계·달력·디렉토리 트리(ncd)·IME 한/영 배지·
  Claude Code 통합·프롬프트 히스토리·세션 리줌·사용 한도 화면·퍼포스 CL 목록·출력
  캡처(REC)·피드백 문구 숨김이 모두 `pytmuxlib/plugins/<name>/` 디렉토리 하나로 응집된
  **플러그인**입니다. 디렉토리를 지우면
  그 기능이 명령·자동완성·렌더 어디에도 안 나타나고 조용히 사라집니다(코어는 레지스트리
  훅 + `getattr` 가드로만 닿음). 만드는 법은 [플러그인 제작 가이드(위키)](https://github.com/neoocean/pytmux/wiki/Plugin-Authoring-Guide).
...
Claude Code 연동, 작업 보존 재시작까지 — 은 **[사용 매뉴얼(위키)](https://github.com/neoocean/pytmux/wiki/User-Manual)**
에 실제 화면 스크린샷과 함께 정리돼 있습니다.

## 문서

- **[사용 매뉴얼(위키)](https://github.com/neoocean/pytmux/wiki/User-Manual) — 📖 상세 사용 매뉴얼(실제 화면 스크린샷 포함). 처음
  쓰신다면 여기부터.**
- [화면 갤러리(위키)](https://github.com/neoocean/pytmux/wiki/Screenshots) — 🖼️ 화면 갤러리: pytmux 본체와 기본 제공 플러그인들의
  주요 화면을 스크린샷 + 짧은 설명으로 한눈에.
- [플러그인 제작 가이드(위키)](https://github.com/neoocean/pytmux/wiki/Plugin-Authoring-Guide) — 🔌 플러그인 매뉴얼: 작성법·훅 계약·
  delete-to-disable 원칙 + 기본 제공 플러그인 사례. 플러그인을 직접 만들려면 여기부터.
