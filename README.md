...
- **상단 탭바**: 탭이 하나여도 상단에 탭 인터페이스(탭 목록 + 마지막 탭 오른쪽의 `[+]` 추가)가
  ...
  닫기). 설정 `set tab-bar auto` 로 탭이 2개 이상일 때만 표시하도록 바꿀 수 있습니다.
  활성 탭은 아래 콘텐츠 영역과 **연결된 노트북 탭 모양**(위쪽 절반 블록 `▀`)으로
  이어지고, 탭을 전환하면 이 연결부도 따라 움직입니다.
...
> 크래시 없이 우아하게 폴백됩니다. (실 Windows 기기 스모크 검증은 진행 중인
> **실험적** 단계입니다.)
...
진입점입니다. macOS/Linux 에 더해 **Windows 네이티브(ConPTY)** 백엔드가 추가되었으며,
실 기기 검증은 진행 중입니다(자세한 범위·현황은 [docs/WINDOWS_PORT.md](docs/WINDOWS_PORT.md)).
헤드리스 테스트는 `python3 tests/run.py` 로 전부 통과합니다(현재 143 passed).
