...
- **Claude Code 상태 표시**: 실행 중인 탭에 상태 아이콘(**대기 `○` / 처리중 `◐` / 리밋
  멈춤 `⊘`**)을 표시하고, 비활성 탭의 작업이 끝나면 탭 배경색으로 알립니다. Claude 패널
  맨 윗줄에는 **지금 처리 중인 프롬프트**를 스티키 헤더로 보여주고, 클릭하면 프롬프트
  히스토리 팝업이 열립니다.
...
- **작업 보존 서버 재시작**: `restart-server`(서버만)·`restart-all`(서버+클라 동시)·
  ...
  새 이미지로 교체**합니다(제자리 re-exec). 자세히는
  [docs/RESTART_SCENARIO.md](docs/RESTART_SCENARIO.md).
...
> 마쳤습니다(자세한 범위는 [docs/WINDOWS_PORT.md](docs/WINDOWS_PORT.md)).
...
- [docs/RESTART_SCENARIO.md](docs/RESTART_SCENARIO.md) — 작업 보존 서버 재시작 설계/명세
- [docs/PERFORMANCE_SCENARIO.md](docs/PERFORMANCE_SCENARIO.md) — 성능/반응성 가속 시나리오
- [docs/WINDOWS_PORT.md](docs/WINDOWS_PORT.md) — Windows 네이티브(ConPTY) 포팅 범위·현황
- [docs/INPUT_FOCUS_NOTE.md](docs/INPUT_FOCUS_NOTE.md) — 명령 프롬프트를 모달 안의 Textual
  Input 으로 띄우는 이유
...
전부 통과합니다(현재 286 passed). 매뉴얼의 스크린샷은 실제 클라이언트를 헤드리스로
운전해 자동 생성하며(`python3 scripts/gen_screenshots.py`), Claude 연동 컷은 진짜
`claude` 를 패널에서 돌려 캡처합니다 — 방식은
[docs/SCREENSHOT_SCENARIO.md](docs/SCREENSHOT_SCENARIO.md) 참고.
