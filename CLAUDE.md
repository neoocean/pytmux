...
## LLM 작업 팁(중요)
- **거대 파일은 부분 읽기**: `client.py`(~4천 줄)·`clientscreens.py`·`tests/test_client.py`
  (~6천 줄)·`tests/test_server.py` 등은 한 컨텍스트에 안 들어온다. `grep -n '^class \|^    def '`
...
- **동적 합성 메서드**: `Server` 의 일부 메서드(`set_autoresume`·`_scan_claude` 등)는
  ...
  주석 참조).
- 명령 라우팅은 명시적 if/elif(동적 디스패치 거의 없음)라 `grep '"split-h"'` 처럼 문자열로
  핸들러를 바로 찾을 수 있다.
