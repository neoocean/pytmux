...
## LLM 작업 팁(중요)
- **거대 파일은 부분 읽기**: `clientscreens.py`(~2.7천 줄)·`plugins/claude-code/screens.py`
  (~2.3천 줄)·`plugins/claude-code/servermixin.py`(~2.2천 줄, 단일 클래스)·`model.py`(~1.4천
  줄)·`serverio.py`(~1.4천 줄)·`tests/test_client.py`(~6.6천 줄)·`tests/test_server.py`(~4.6천
  줄) 등은 한 컨텍스트에 안 들어온다. `grep -n '^class \|^    def '` 로 위치를 잡고 Read
  offset/limit 으로 관심 영역만 읽어 부분 수정→회귀를 피한다. (`client.py` 는 믹스인 3모듈로
