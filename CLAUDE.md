...
- ★★ **작업을 시작할 때 자기 numbered CL 을 먼저 만들고 거기에 연다**(사용자 지시 2026-09-03).
  `p4 change -i` 로 CL 을 낸 뒤 `p4 edit -c <번호>` 또는 `p4 reconcile -c <번호>` 로 연다.
  ⛔ **맨 `p4 edit`(`-c` 없음)은 쓰지 않는다** — 그것은 default CL 로 가고 거기서는 어느 세션의
  작업인지 아무도 못 가린다.
  ☠ **default 에 두면 남이 치운다 — 2026-09-03 에 그렇게 됐다.** 사람이 'default CL 금지' 규칙만
  보고 진행 중이던 파일 여덟을 numbered CL 로 옮겼고 그 순간 이 저장소의 마무리가 막혔다:
  `p4 reconcile` 은 **남의 CL 이 쥔 파일을 못 연다**(정본은 issue 저장소
  `src/backupstate.mjs` §otherHolders) — 그러면 마무리가 '담을 것이 없다' 를 찍으며 **성공으로
  나간다**. 되돌리고 이 규칙을 세운 것이 그 회차다.
  ★ 덤으로 아래 두 함정이 함께 사라진다 — 내 것만 열려 있으므로 새 CL 이 남의 파일을 쓸어 갈 일이
  없고 게시 전 부정 게이트가 걸릴 일도 없다(그래도 게이트는 백스톱으로 남긴다).
